...
struct SlopeOutputs {
    ...
    // §53k cold-rate fall speeds (Fortran F:1952-1955: vt2r/vt2s/vt2i = DBLE(pvt)·rslopeb·denfac,
    // recomputed UNCONDITIONALLY per rate-loop cell — NO q<=0 zeroing, unlike the sed vt_*; empty
    // cells carry the inactive-branch rslopeb*max values, which the |vt2x−vt2y| cancellations in
    // nsaci/ngaci/... require exactly).
    torch::Tensor vt2r, vt2s, vt2i;
