...
ProgBOutputs progb_param_torch(
    ...
    // §53b op-path RAW division (Fortran F:3569 rhox = qrs(3)/brs, NO divisor floor): after the
    // melt block a cell can carry brs = -Inf / negative / qNaN (F:1384 pgmlt/rhox with the
    // persistent rhox=0). Fortran then gets rhox = qg/-Inf = -0 → max/min (fmaxnm/fminnm on
    // aarch64) clamps to RHO_MIN=100 (and NaN → 100), whereas the BRS_MIN-clamped divisor sent
    // those cells to RHO_MAX=900 — a 9× brs=qg/rhox split (the "177×catastrophic" class).
