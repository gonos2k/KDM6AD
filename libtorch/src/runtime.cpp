...
CoordinatorForcing build_forcing(const Forcing& f) {
    // `dend` is air density rho [kg m^-3], matching Fortran F:812 and the
    // Python CoordinatorForcing contract. Sedimentation forms the volumetric
    // fall flux falk = rho*q*(vt/delz); surface accumulation applies the
    // bottom-layer delz exactly once.
...
// A Tensor carries the shape metadata that the raw C xland pointer cannot.
// Validate direct C++ callers before flattening/broadcasting so a short mask
// cannot be read as though it had im*jme entries and NaN cannot silently land
// in the land/sea comparison (where it would otherwise classify as land).
void validate_xland(const torch::Tensor& xland, int64_t ncol) {
    TORCH_CHECK(xland.defined(), "xland must be a defined tensor");
    if (xland.dim() == 1) {
        TORCH_CHECK(xland.numel() == ncol,
                    "xland 1-D cardinality must be im*jme (", ncol,
                    "), got ", xland.numel());
    } else {
        TORCH_CHECK(xland.dim() == 2 && xland.numel() == ncol,
                    "xland 2-D cardinality must be im*jme (", ncol,
                    "), got ", xland.sizes());
    }
    torch::NoGradGuard no_grad;
    TORCH_CHECK(torch::isfinite(xland).all().item<bool>(),
...
FnResult kdm6_fn(const State& state,
    ...
    // Fail-loud variant gate FIRST — before any State/Forcing work and before
    ...
    // delt<=0 → exact no-op.  Return before state/forcing coordinate conversion:
    // even a mathematically cancelling t=th*pii; th=t/pii round trip can move a
    // float32 theta by one ULP.  The original State tensors are the identity map.
    if (dt <= 0.0) {
        auto z = torch::zeros({state.qc.size(0)}, state.qc.options());
        return FnResult{/*state_out=*/state,
                        /*rain_increment=*/z, /*snow_increment=*/z, /*graupel_increment=*/z,
                        /*rhog=*/torch::zeros_like(state.qg)};
    }
...
StepResult kdm6_step(const State& state, const Forcing& forcing,
                     ...
                     const PhysicsOptions& physics) {
    // Fail-loud variant gate FIRST (C3) — shared helper, identical behavior to
    // the previous inline switch (see validate_physics_variant above).
    validate_physics_variant(physics.variant, "kdm6_step");
