...
torch::Tensor wilt_reduction(const torch::Tensor& ratio) {
    // §53w: Fortran `min(max(0.0, ratio), 1.)` lowers to fmaxnm/fminnm on aarch64 —
    // NaN is IGNORED (0/0 raw Wilt ratio → max(0,NaN)=0 → wilt=0). torch::clamp
    // PROPAGATES NaN, so a gate-true cell with BOTH species zero (e.g. qi=0 & qr=0
    // with nr>nrmin & ni>ncmin — niacr/nraci) leaked NaN into the nr/ni budget
    // sources → budget factors NaN → every nr/ni-budget rate NaN (step-22 wrapper
    // fatal). fmax/fmin are the IEEE maximumNumber/minimumNumber ops (NaN-ignoring),
    // matching gfortran exactly; +Inf→1 and normal values are unchanged.
    auto zero = torch::zeros_like(ratio);
    auto one  = torch::ones_like(ratio);
    auto clamped = torch::fmin(torch::fmax(ratio, zero), one);
    return clamped * clamped;
}

// §53n: Fortran computes the Wilt collection-efficiency ratio with RAW q divisions
// (e.g. F:1982 qrs(1)/qci(2)) — the *_safe qcrmin floors distorted the sub-qcrmin
// tails by ORDERS of magnitude (piacr F=1e-25 vs C=6e-17; the poststate qr-11/brs-9
// class). Raw ratio on the f32 op path (each rate's gate keeps its live denominator
// species positive; dead-branch Inf/NaN is where()-masked); f64 DA path keeps the
// clamped-safe ratio (finite adjoint).
...
IceAccretionParams default_ice_accretion_params() {
    ...
    // §53m rain d-gammas via the gfortran GAMMLN f32 mirror (Fortran F:3376/3423-3424
    // g*pdrmr = rgmma(drmur*) REAL(4); non-integer Γ(5)/Γ(6)/Γ(7) differ from
    // exp(lgamma(double)) at ~1e-7 relative — the piacr 788-cell f32-store-flip class).
    const double g1pdrmr = fconst::get().g1pdrmr;
    const double g2pdrmr = fconst::rgmma_f(2.0f + static_cast<float>(constants::DMR) + static_cast<float>(constants::MUR));
    const double g3pdrmr = fconst::rgmma_f(3.0f + static_cast<float>(constants::DMR) + static_cast<float>(constants::MUR));
...
IceAccretionOutputs ice_accretion_torch(
    ...
    // §53l f32-stepwise prefix (F:1977 `(pi)*cmi` REAL(4) before the f64 n0i factor) — the
    // mass sibling of §53h; the f64 scalar-fold was 1 ULP off (praci/piacr/psaci/pgaci class).
    const double praci_pref = static_cast<double>(
        static_cast<float>(fconst::get().pi) * static_cast<float>(p.cmi));
    auto praci_raw = (praci_pref * in.n0i * in.n0r * torch::abs(in.vt2r - in.vt2i)
                     / (4.0 * den_safe) * acrfac_pr * p.eacri).to(in.qi.scalar_type());
    auto praci_wilt = praci_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qr, in.qi, qr_safe, qi_safe));
    ...
    const double piacr_pref = static_cast<double>(
        static_cast<float>(fconst::get().pi) * static_cast<float>(p.cmr));  // §53l (F:1992 (pi)*cmr)
    auto piacr_raw = (piacr_pref * in.n0i * in.n0r * torch::abs(in.vt2i - in.vt2r)
                     / (4.0 * den_safe) * acrfac_pi * p.eacir).to(in.qr.scalar_type());  // f32 store F:1892
    auto piacr_wilt = piacr_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qi, in.qr, qi_safe, qr_safe));
...
IceToSnowGraupelOutputs ice_to_snow_graupel_torch(
    ...
    auto psaci_raw =
        (static_cast<double>(static_cast<float>(fconst::get().pi) * static_cast<float>(p.cmi))  // §53l (F:2006 (pi)*cmi)
         * in.n0i * in.n0so * in.n0sfac * torch::abs(in.vt2s - in.vt2i)
        / (4.0 * den_safe) * acrfac_s * eac_temp).to(in.qi.scalar_type());  // f64 n0i chain, f32 store F:1906
    auto psaci_wilt = psaci_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qs, in.qi, qs_safe, qi_safe));
    ...
    auto pgaci_raw =
        (static_cast<double>(static_cast<float>(fconst::get().pi) * static_cast<float>(p.cmi))  // §53l (F:2026 (pi)*cmi)
         * in.n0i * in.n0go * torch::abs(in.vt2g - in.vt2i)
        / (4.0 * den_safe) * acrfac_g * eac_temp).to(in.qi.scalar_type());  // f64 n0i chain, f32 store F:1921
    auto pgaci_wilt = pgaci_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qg, in.qi, qg_safe, qi_safe));
...
NumberAccretionOutputs number_accretion_torch(
    ...
    auto nraci_wilt = nraci_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qr, in.qi, qr_safe, qi_safe));
    ...
    auto niacr_wilt = niacr_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qi, in.qr, qi_safe, qr_safe));
    ...
    auto nsaci_wilt = nsaci_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qs, in.qi, qs_safe, qi_safe));
    ...
    auto ngaci_wilt = ngaci_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qg, in.qi, qg_safe, qi_safe));
...
#ifdef KDM6_SUBSTEP_DUMP
    // nsaci/ngaci forensic: mirror fort_ncrates tail fields (f32 casts of the f64 locals).
    // Same call-counter gate idiom as the coordinator entry dump (call N == step N × tile).
    {
        static long ncacc_call = 0;
        ++ncacc_call;
        static const long ncacc_target = []{ const char* e = std::getenv("KDM6_DUMP_CALL"); return e ? std::atol(e) : 1L; }();
        const char* env = std::getenv("KDM6_SUBSTEP_DUMP");
        if (env != nullptr && ncacc_call == ncacc_target) {
            std::string path = std::string(env) + "/cpp_ncacc.bin";
            std::ofstream f(path, std::ios::binary);
            if (f) {
                auto dump = [&](const torch::Tensor& t){
                    auto c = t.detach().to(torch::kFloat32).contiguous().cpu();
                    const int64_t n = c.numel(); const float* p2 = c.data_ptr<float>();
                    std::vector<uint32_t> buf(static_cast<size_t>(n));
                    for (int64_t q=0;q<n;++q){uint32_t u;std::memcpy(&u,&p2[q],4);buf[static_cast<size_t>(q)]=__builtin_bswap32(u);}
                    f.write(reinterpret_cast<const char*>(buf.data()), n*4);
                };
                int32_t B = static_cast<int32_t>(in.ni.size(0));
                int32_t K = static_cast<int32_t>(in.ni.size(1));
                ...
                dump(in.vt2s); dump(in.vt2i); dump(acrfac_nsa); dump(acrfac_nga);
                dump(in.n0i); dump(in.n0sfac); dump(eac_temp); dump(in.vt2g);
            }
        }
    }
#endif
...
CloudWaterRimingOutputs cloud_water_riming_torch(
    ...
    auto psacw_raw =
        ...
        * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qs, in.qc, in.qs, qc_safe))
        * in.qc * in.denfac;
    ...
    // §53h f32-stepwise scalar prefix (F:2089 `(pi)*avts*.25*eacsc` is REAL(4) stepwise before
    // the ×n0so tensor factor; the compile-time f64 fold was 1 ULP off — the nsacw→naacw
    // 7162-cell number-riming class).
    const double nsacw_pref = static_cast<double>(
        ((static_cast<float>(fconst::get().pi) * static_cast<float>(p.avts)) * 0.25f)
        * static_cast<float>(p.eacsc));
    auto nsacw_raw =
        (nsacw_pref * in.n0so * in.n0sfac * in.n0c / (p.muc + 1.0)
        * p.g3pbs
        * in.rslope3_s * in.rslopeb_s * in.rslopemu_s
    ...
    auto pgacw_raw =
        ...
        * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qg, in.qc, in.qg, qc_safe))
        * in.denfac;
    ...
    // §53h f32-stepwise scalar prefix (F:2150 `pi*avti*.25*eacic` REAL(4) stepwise — same
    // class as nsacw; the niacw 610-cell class).
    const double niacw_pref = static_cast<double>(
        ((static_cast<float>(fconst::get().pi) * static_cast<float>(p.avti)) * 0.25f)
        * static_cast<float>(p.eacic));
    auto niacw_raw =
        (niacw_pref * in.n0i * in.n0c / (p.muc + 1.0)
        * p.g3pbi
        * in.rslope3_i * in.rslopeb_i * in.rslopemu_i
        * in.rslopec * in.rslopecmu
        * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qi, in.qc, in.qi, qc_safe))
...
RainSnowGraupelCollectionOutputs rain_snow_graupel_collection_torch(
    ...
    auto pracs_raw = (
        static_cast<double>(static_cast<float>(fconst::get().pi) * static_cast<float>(p.cms))  // §53l
         * in.n0so * in.n0sfac * in.n0r * torch::abs(in.vt2r - in.vt2s)
        / (4.0 * den_safe) * acrfac_pracs * p.eacrs).to(in.den.scalar_type());  // f64 n0so/n0r, f32 store
    auto pracs_wilt = pracs_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qr, in.qs, qr_safe, qs_safe));
    ...
    auto psacr_wilt = psacr_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qs, in.qr, qs_safe, qr_safe));
    ...
    auto nsacr_wilt = nsacr_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qs, in.qr, qs_safe, qr_safe));
    ...
    auto pgacr_wilt = pgacr_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qg, in.qr, qg_safe, qr_safe));
    ...
    auto ngacr_wilt = ngacr_raw * wilt_reduction(wilt_arg(in.den.scalar_type() == torch::kFloat32, in.qg, in.qr, qg_safe, qr_safe));
...
DepSubOutputs dep_sub_torch(
    ...
    auto coeres_s = in.rslope2_s
                  * torch::sqrt((in.qs.scalar_type() == torch::kFloat32) ? in.rslope_s * in.rslopeb_s : torch::clamp(in.rslope_s * in.rslopeb_s, /*min=*/p.qcrmin))  /* §53u raw (F coeres) */
                  * in.rslopemu_s;
    ...
    auto coeres_g = in.rslope2_g
                  * torch::sqrt((in.qg.scalar_type() == torch::kFloat32) ? in.rslope_g * in.rslopeb_g : torch::clamp(in.rslope_g * in.rslopeb_g, /*min=*/p.qcrmin))  /* §53u raw (F coeres) */
                  * in.rslopemu_g;
