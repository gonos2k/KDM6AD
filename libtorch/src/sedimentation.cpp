...
void require_column_shapes(const torch::Tensor& reference,
                           std::initializer_list<torch::Tensor> tensors,
                           const torch::Tensor& mstep_col) {
    TORCH_CHECK(reference.dim() == 2 && reference.size(0) > 0 && reference.size(1) > 0,
                "sedimentation state must have positive (B, K) shape");
    for (const auto& tensor : tensors)
        TORCH_CHECK(tensor.sizes() == reference.sizes(),
                    "all sedimentation state/work/fall/forcing shapes must match (B, K)");
    TORCH_CHECK(mstep_col.dim() == 1 && mstep_col.size(0) == reference.size(0),
                "mstep_col must have shape (B,)");
}
...
SubstepAdvectionOutputs substep_advection_torch(
    ...
    require_column_shapes(in.state.qr, {
...
IceSubstepOutputs ice_substep_advection_torch(
    ...
    require_column_shapes(in.state.qi, {
