...
hydro size param            ClwdeParam/IcedeParam = user-Deff (reff→Deff 입력; number-moment 경로)
...
- **보간**: model-level→RTTOV grid 로그압력 보간은 per-profile 1회 계산한 **상수 torch 행렬**.
  **ami/501 coef는 54 levels / 53 layers**(69 아님, 정정 Round-2): `W_layer`(→53층, T/Q/content/Deff)와
  `W_level`(→54준위, PHalf)로 분리. Wᵀ가 autograd 자동 합성 — numpy 보간 금지.
- p/PHalf는 **forcing 상수(미분 대상 아님)** — Pa→hPa 스케일링은 numpy pack 단계에 둬도 무방
  (상수라 grad 불필요). 단조성 검증(M2); 외삽 금지.
- P/T는 LAYERS(53), PHalf는 LEVELS(54) — RTTOV-14 규약.
...
계약 (interface contract, pyrttov 실측 기반):
  ...
  6. P/T는 LAYERS(nlayers), PHalf는 LEVELS(nlevels) — shape 혼동 금지.
```
...
그리고 RTTOV-K가 ∂BT/∂Deff(getHydroDeffNK)를 주는 것은 **Deff를 EXPLICIT 입력 profile item으로
넣었을 때뿐**이다(reff_liq→setHydroDeffN(.,6), reff_ice→setHydroDeffN(.,7)). 만약
`ClwdeParam/IcedeParam`을 pyrttov 기본값(=내부 파라미터화)으로 두면 **λ_Deff≡0 → λ_nc=λ_ni≡0**.
...
**M5에서 config(ClwdeParam=user) 단언 + getHydroDeffNK 비영(非零) probe**로 잡는다(아래 §10 주).
...
config(ClwdeParam/IcedeParam=user) 단언 + getHydroDeffNK 비영 probe**로 한다(§9.1/§14.5).
...
test_rttov_k_cloud_fd                  dBT/dcontent vs FD (getItemK HYDRO1-5)
test_rttov_k_deff_fd                   dBT/dDeff vs FD (getHydroDeffNK; ★nc/ni number-moment 경로)
test_user_deff_mode_or_fail            ClwdeParam/IcedeParam=user 아니면 hard-fail (λ_nc/λ_ni 생존)
...
| **입자크기 미분 경로 단절** (blocking) | user-Deff 아니면 λ_nc=λ_ni≡0(nc/ni만; nr/bg는 RTTOV Deff 없어 obs-path 자체 없음) | reff→Deff(×2) EXPLICIT 입력 + ClwdeParam/IcedeParam=user; M5 비영 probe(covector 못 잡음) |
| **covector 무경로 vs 단절 혼동** (major) | nr/bg를 필수로 요구하면 매 호출 false-raise | nr/bg/nccn=OBS_ZERO_OK(0 정답); 단절(numpy)만 raise(§10) |
| **단위/보간 Jacobian이 torch 밖** (blocking) | λ_qv·λ_x에서 ∂Q/∂qv·∂(interp) 누락 | qv→Q·reff→Deff·보간(54lev/53lay)을 torch 연산(상수 W)으로; cloud content는 bridge가 이미 g/m³ — 재변환 금지 §14.3-units |
...
#    gas/T: extract_columns + (qv→Q ppmv-moist) + (보간 W_layer→53층 / W_level→54준위)
...
class RttovObsOp(torch.autograd.Function):
  ...
  forward(ctx, T_lay, Q_lay, *cloud_and_surface_tensors):
      ...
      ctx.K = read_K_accessors(out)               # dict{'T','Q','HYDRO4',...,'HYDRO_DEFF6/7'}
  ...
  backward(ctx, lambda_BT):
      ...
      #   grad_clw   = einsum('pcl,pc->pl', Kt['HYDRO4'],     lambda_BT)  # content
      #   grad_deff_liq = einsum('pcl,pc->pl', Kt['HYDRO_DEFF6'], lambda_BT)  # 입자크기
...
- **입력 단위/슬롯 계약**: `Profiles.GasUnits = 2(ppmv_wet)` (pyrttov 기본 1=kg/kg),
  ...
  동급으로 "명시 또는 실패".
- **user-Deff 모드 강제 (number-moment 경로 생존 조건)**: `ClwdeParam/IcedeParam`을 user-Deff로 두고
  reff_*→Deff(×2)를 `setHydroDeffN(.,6/.,7)`로 EXPLICIT 입력. 기본(내부 파라미터화)이면 λ_nc=λ_ni≡0 →
  M5에서 hard-fail.
