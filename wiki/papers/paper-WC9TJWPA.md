...
이 페이지는 Milbrandt 2016 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** P3 Part III는 single free ice category의 property dilution 문제를 줄이기 위해 multiple free ice categories를 도입한다.

## 기본 서지와 근거 상태

- Zotero key: `WC9TJWPA`
- 연도: `2016`
- 저자: Milbrandt, J. A.; Morrison, H.
- DOI/URL: `10.1175/jas-d-15-0204.1`
- 컬렉션 섹션: `원문 보강 필요`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: 원문 PDF 미확보. AMETSOC 공개 abstract·metadata 기준: https://doi.org/10.1175/JAS-D-15-0204.1

## 학술적 가치

- bulk microphysics에서 “category를 없애자”는 방향이 다시 “multiple free categories”로 돌아오는 이유를 보여준다. 이것은 category와 property prediction 사이의 균형 문제다.
- KDM6AD가 fixed categories를 미분가능하게 만들 때, category 수와 property dilution/mixture 문제가 gradient 해석에도 영향을 준다는 점을 설명한다.
- D629MKTV graupel density와 함께 frozen particle property representation의 최신 비교축이다.
...
- P3 Part I/II 이후 multiple free categories로 확장되는 후속 문헌이다.
- Milbrandt/Morrison 계열 multimoment/property prediction 설계와 연결된다.
- KDM6AD의 category-based WDM/KDM 계열과 대조되는 현대 ice microphysics 계보다.

섹션 계보 요약: 불완전 근거 문헌은 확정 주장보다 “보강해야 할 계보 연결”로 취급한다. 특히 Seifert-Beheng two-moment, P3, KIM, WSM7 hail은 KDM6AD 설명에서 연결성이 크다.

이 위치 때문에 Milbrandt 2016은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- 서로 다른 bulk property set을 가진 ice particle populations가 공존하도록 multiple free categories를 둔다.
- single category에서 서로 다른 particle types가 섞여 property dilution이 발생하는 문제를 줄인다.
- scheme behavior와 idealized/application behavior를 설명한다.
...
- state가 category index c와 property moments p_c를 함께 가지면 Jacobian은 block-structured가 된다.
- category splitting/merging 또는 activation/deactivation은 differentiability에서 event-like discontinuity를 만들 수 있다.
- KDM6AD가 fixed q_i/q_s/q_g categories를 쓰는 경우와 P3 multiple free categories는 서로 다른 AD sparsity를 가진다.
...
- future KDM6AD 확장에서 multiple ice-property state를 고려할 때 ABI와 gradient validation 난이도를 예측하게 해 준다.
- VJP의 physical attribution은 category definitions에 강하게 의존함을 보여준다.
- 원문 미확보라 세부 실험 결론은 보류한다.
...
- ice category/property 설계는 미세물리 gradient의 해석 단위를 결정한다.
- KDM6AD는 fixed-category 계보의 AD 구현으로, P3 multiple-category 계보와 비교할 때 학술적 위치가 선명하다.
...
- 공개 abstract/metadata 기반 보수 정리다.
- KDM6AD 현행 구현과 직접 대응시키면 안 된다.
...
| 축 | Milbrandt 2016에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Milbrandt 2016을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Milbrandt 2016을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Milbrandt 2016의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
