...
이 페이지는 Grundner 2024 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** cloud cover parameterization을 data-driven equation discovery로 재구성해, 미분가능 모델이 물리 parameterization의 형태 자체를 탐색할 수 있음을 보여준다.

## 기본 서지와 근거 상태

- Zotero key: `JSEKWT8Q`
- 연도: `2024`
- 저자: Grundner, Arthur; Beucler, Tom; Gentine, Pierre; Eyring, Veronika
- DOI/URL: `10.48550/arxiv.2304.08063`
- 컬렉션 섹션: `자동미분·미분가능 구현`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`125313` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD가 기존 KDM6 연산자를 미분가능하게 만드는 작업이라면, 이 논문은 그 다음 단계인 differentiable equation discovery/closure learning의 비교축이다.
- cloud cover는 microphysics와 radiation/large-scale dynamics를 잇는 parameterization이므로 KDM6AD의 cloud-process gradient와 연결 가능하다.
- 학술적으로 “AD는 기존 scheme의 미분”과 “AD를 통한 새 closure 탐색”이라는 두 경로를 구분하게 해 준다.
...
- Differentiable ESM과 hybrid AI climate model 계보 속에서 equation discovery 쪽 결절점이다.
- Morrison CAM two-moment, partial cloudiness, cloud fraction closure 문헌과도 연결된다.
- KDM6AD는 equation discovery의 대상 데이터를 만들거나 differentiable baseline process로 사용될 수 있다.
...
이 위치 때문에 Grundner 2024은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- data-driven equation discovery로 cloud cover parameterization 형태를 찾거나 개선한다.
- differentiable optimization을 통해 candidate equations/parameters를 선택한다.
- physical interpretability와 predictive skill 사이의 균형을 다룬다.
...
- closure C=f(x;theta) 또는 discovered symbolic form에서 dL/dtheta를 AD로 계산한다.
- KDM6AD의 F_micro(y)와 cloud cover closure C(y) 사이가 연결되면 dL/dy는 C_y와 F_y를 거쳐 전파된다.
- 형태 탐색은 gradient correctness뿐 아니라 identifiability와 regularization 문제를 포함한다.
...
- KDM6AD의 gradients를 이용해 microphysics-informed cloud closure learning을 future work로 제시할 수 있다.
- 기존 scheme parity를 먼저 확보한 뒤 equation discovery를 얹어야 scientific baseline이 흔들리지 않는다.
- KDM6AD 자체를 data-driven parameterization이라고 부르면 안 된다.
...
- AD는 legacy scheme sensitivity뿐 아니라 closure discovery에도 쓰인다.
- KDM6AD는 학습 기반 cloud/process closure 연구의 differentiable physical backbone 후보가 될 수 있다.
...
- cloud cover closure와 KDM6 microphysics는 다른 parameterization layer다.
- equation discovery 결과를 KDM6AD 구현의 물리 검증으로 사용할 수 없다.
...
| 축 | Grundner 2024에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Grundner 2024을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Grundner 2024을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Grundner 2024의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
