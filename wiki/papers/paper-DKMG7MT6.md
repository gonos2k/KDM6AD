...
이 페이지는 Milbrandt 2015 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** P3 Part I는 고정 ice category 대신 bulk ice particle properties를 예측해 ice microphysics category problem을 재구성한 중요한 전환점이다.

## 기본 서지와 근거 상태

- Zotero key: `DKMG7MT6`
- 연도: `2015`
- 저자: Milbrandt, Jason A.; Morrison, Hugh
- DOI/URL: `10.1175/jas-d-14-0065.1`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`141495` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD가 WDM/KDM category scheme을 미분가능하게 만든다면, P3는 category 자체를 물리 property prediction으로 바꾸는 대안 계보다.
- rime mass, rime volume, total ice mass, number 같은 property state는 graupel density prediction과 직접 비교된다.
- AD 관점에서는 categorical branch를 줄이고 continuous property state를 늘리는 설계가 gradient smoothness에 어떤 장단점을 갖는지 보여준다.
...
- Lin/Thompson/WSM/WDM의 fixed ice category 전통에 대한 대안이다.
- Part II case study와 Part III multiple free categories로 이어진다.
- D629MKTV graupel density prediction은 WDM 계열 안에서 P3적 문제의식을 부분적으로 수용한 것으로 읽을 수 있다.
...
이 위치 때문에 Milbrandt 2015은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- all ice-phase particles를 predefined snow/graupel/hail category가 아니라 예측되는 bulk properties로 표현한다.
- total ice mass, rime mass, rime volume, number 등을 통해 particle density와 fall behavior를 진단한다.
- idealized tests로 property prediction scheme의 behavior를 검토한다.
...
- state vector에 q_i, q_rime, V_rime, N_i가 들어가면 density rho_i=q_rime/V_rime 같은 derived quantity가 source/fall-speed에 들어간다.
- category conversion branch 일부가 continuous property evolution으로 바뀌면 gradient가 더 해석 가능해질 수 있지만, positivity limiter와 ratio singularity가 생긴다.
- KDM6AD의 graupel density diagnostic과 수학적으로 비교할 수 있다.
...
- KDM6AD의 장기 확장 방향 중 하나는 fixed-category KDM을 P3-style property-sensitive differentiable scheme과 비교하는 것이다.
- VJP에서 reflectivity/precipitation loss가 particle property state로 돌아가는 구조를 설명한다.
- 현행 KDM6AD에 P3를 넣었다고 쓰지 말고, 대안 계보로 써야 한다.
...
- ice category problem은 bulk microphysics의 핵심 학술 문제다.
- KDM6AD의 frozen hydrometeor AD는 P3 계보와 비교될 때 가치와 한계가 분명해진다.
...
- P3는 KDM6/WDM6의 직접 구현이 아니다.
- continuous property prediction이 항상 더 미분가능하다는 단순 주장은 ratio/limiter 문제를 무시한다.
...
| 축 | Milbrandt 2015에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Milbrandt 2015을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Milbrandt 2015을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Milbrandt 2015의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
