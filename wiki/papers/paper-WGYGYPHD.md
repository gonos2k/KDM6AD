...
**핵심 판정:** WSM7은 WSM6에 prognostic hail hydrometeor를 추가해 hail/graupel distinction과 fast-falling ice의 중요성을 보여주는 WRF single-moment 계열 문헌이다.

## 기본 서지와 근거 상태

- Zotero key: `WGYGYPHD`
- 연도: `2018`
- 저자: Hong, Song-You; Bae, Soo Ya; Tao, Wei-Kuo
- DOI/URL: `10.1007/s13143-018-0066-3`
- 컬렉션 섹션: `원문 보강 필요`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: 원문 PDF 미확보. Springer 공개 Abstract 기준: https://link.springer.com/article/10.1007/s13143-018-0066-3

## 학술적 가치

- WDM7, graupel density prediction, P3와 함께 frozen hydrometeor category refinement 계보를 구성한다.
- KDM6AD가 graupel/hail/diagnostic density를 논할 때 “왜 hail-like fast falling category가 필요한가”를 설명한다.
- single-moment scheme이지만 category addition이 precipitation distribution을 크게 바꿀 수 있음을 보여준다.
...
- WSM6 -> WSM7 single-moment hail extension으로 이어진다.
- WDM7 revision과 D629MKTV의 WDM6 graupel density prediction과 같은 문제의식을 공유한다.
- KDM6AD의 fixed KDM6 state와 future hail/graupel extension 사이의 비교축이다.
...
- hail prognostic hydrometeor를 추가해 graupel과 hail의 sedimentation/fall-speed 차이를 표현한다.
- WRF simulations에서 leading-edge convective activity, trailing stratiform precipitation, precipitation intensity distribution 변화를 평가한다.
- single-moment 구조를 유지하면서 category를 추가한다.
...
- 새 hydrometeor q_h가 추가되면 state dimension과 source conversion matrix가 바뀐다.
- hail fall speed가 크면 sedimentation operator와 surface precipitation sensitivity가 달라진다.
- KDM6AD ABI가 q_h를 포함하지 않는다면 이 gradient는 현행 state 밖에 있다.
...
- KDM6AD의 future WDM7/hail extension에서 ABI-breaking state addition 예시다.
- radar/precipitation DA에서 hail/graupel distinction이 observation operator sensitivity를 바꿀 수 있음을 보여준다.
...
- frozen hydrometeor category addition은 forecast와 AD state structure를 동시에 바꾼다.
- KDM6AD의 state/ABI 정의는 향후 hail/graupel extension 여부와 함께 관리되어야 한다.
...
- WSM7은 single-moment이고 KDM6AD 현행 대상과 다르다.
- prognostic hail 추가 결과를 KDM6AD의 현재 성능으로 사용하면 안 된다.
