...
이 페이지는 Vié 2016 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** LIMA는 aerosol population과 hydrometeor number를 결합한 quasi two-moment scheme으로, WDM6보다 aerosol-cloud 상호작용을 더 넓게 다루는 계보적 비교점이다.

## 기본 서지와 근거 상태

- Zotero key: `S98KNIGB`
- 연도: `2016`
- 저자: Vié, B.; Pinty, J.-P.; Berthet, S.; Leriche, M.
- DOI/URL: `10.5194/gmd-9-567-2016`
- 컬렉션 섹션: `KDM/WDM 계열 핵심 미세물리`
- 컬렉션 내 역할: CCN/aerosol 및 수농도 민감도 축
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`120845` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD가 CCN/Nc/Nr gradient를 논할 때, LIMA는 “aerosol-aware two-moment microphysics”의 더 일반화된 설계공간을 제공한다.
- bulk scheme이 단일 empirical parameterization을 넘어 aerosol mode, activation, ice-nucleating particle까지 상태화할 수 있음을 보여준다.
- 미분가능 구현 관점에서는 많은 mode와 activation branch가 AD에 어떤 부담을 주는지 비교하는 데 중요하다.
...
- WDM6의 prognostic CCN 축과 Seifert/Beheng mixed-phase two-moment 계보를 유럽 mesoscale model 계열에서 확장한다.
- SuperdropNet/ML microphysics와 연결하면 aerosol-cloud process를 surrogate 또는 differentiable component로 다루는 현대 계보로 이어진다.
- KDM6AD의 초기 목표가 WDM/KDM parity라도, 장기적으로 aerosol-aware DA로 갈 때 비교 기준이다.
...
이 위치 때문에 Vié 2016은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- cloud droplets, raindrops, aerosols, ice nuclei를 quasi two-moment 구조로 연결한다.
- activation/nucleation이 prescribed concentration이 아니라 prognostic 또는 modal population과 연결된다.
- bulk computational cost를 유지하면서 aerosol-cloud-precipitation interaction을 explicit하게 만든다.
...
- NCCN -> Nc activation mapping은 KDM6AD의 핵심 gradient와 같은 수학적 가족이다.
- mode-resolved aerosol moments가 많아질수록 Jacobian은 sparse하지만 구조적으로 커지고, threshold activation은 piecewise differentiability 문제가 된다.
- VJP 해석에서는 강수 오차가 aerosol mode로 역전파될 때 물리적 identifiability가 약해질 수 있다.
...
- KDM6AD의 CCN 민감도가 독립적 연구질문임을 강화한다.
- future work에서 aerosol-aware control variables를 확장할 때 선행 문헌으로 쓸 수 있다.
- 단, LIMA 자체 구현과 KDM6AD 구현을 직접 비교하면 scheme 차이가 너무 크므로 설계공간 비교로 제한한다.
...
- prognostic aerosol/CCN 변수는 미세물리 민감도와 자료동화의 자연스러운 제어축이다.
- activation/nucleation branch의 미분가능성은 KDM6AD 장기 확장에서 핵심 기술문제다.
...
- KDM6AD의 WDM6-derived state와 LIMA의 modal aerosol state는 일대일 대응하지 않는다.
- 원문 실험 결과를 KDM6AD 성능 주장으로 가져오면 안 된다.
...
| 축 | Vié 2016에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Vié 2016을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Vié 2016을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Vié 2016의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
