...
**핵심 판정:** three-moment closure와 scheme description을 통해 hydrometeor spectrum을 mass/number/reflectivity 등 더 많은 moment로 제어하는 설계공간을 제시한다.
...
- DOI/URL: `10.1175/jas3535.1`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`110076` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD가 double-moment WDM/KDM 계열에 머물더라도, 더 높은 moment closure가 어떤 정보를 추가하는지 비교 기준을 제공한다.
- reflectivity-sensitive DA를 논할 때 M6-like moment를 직접 예측하는 scheme과 진단하는 scheme의 차이를 설명할 수 있다.
- bulk microphysics가 계산비용과 spectral fidelity 사이에서 어떤 절충을 하는지 보여준다.
...
- Part I의 spectral shape parameter 분석을 scheme 구현으로 확장한다.
- Mansell sedimentation/advection, electrification, radar DA 문헌과 moment transport 계보를 공유한다.
- P3와는 다른 방식으로 “더 많은 bulk property를 예측”하는 흐름이다.
...
- three-parameter gamma PSD에서 세 개의 prognostic moments를 이용해 N0, lambda, shape을 더 자유롭게 결정한다.
- sedimentation과 growth 과정에서 size sorting을 더 잘 표현하려 한다.
- scheme description은 practical model implementation의 process decomposition을 제시한다.
...
- M0, M3, M6 같은 moment set이 PSD closure를 정한다. reflectivity Z는 대략 D^6 moment에 민감하므로 third moment choice는 observation operator와 직접 연결된다.
- KDM6AD가 M6를 진단할 때와 prognostic state로 둘 때 dZ/dy 구조가 다르다.
- three-moment scheme은 Jacobian 차원이 커지지만 관측공간 sensitivity의 해석 가능성은 커질 수 있다.
...
- KDM6AD의 current state가 double-moment라는 점의 한계를 설명하는 비교문헌이다.
- radar reflectivity DA 확장 시 “진단 reflectivity gradient”와 “prognostic reflectivity moment gradient”의 차이를 논할 수 있다.
- higher-moment extension은 ABI와 memory layout을 크게 바꾸므로 후속 연구로 분리해야 한다.
...
- moment 개수는 미세물리 scheme의 물리 fidelity와 AD Jacobian 구조를 동시에 바꾼다.
- radar-oriented DA에서는 moment closure 선택이 observation sensitivity의 핵심이다.
...
- KDM6AD가 three-moment scheme이라는 뜻은 아니다.
- 세 모멘트가 항상 더 낫다는 일반 주장을 하면 안 된다. 계산비용과 closure uncertainty가 함께 증가한다.
