...
이 페이지는 Baumgartner 2019 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** IFS cloud scheme에 CoDiPack 기반 algorithmic differentiation을 적용해 cloud microphysics parameter sensitivity를 체계적으로 산출한, KDM6AD의 직접적인 방법론 선행문헌이다.

## 기본 서지와 근거 상태

- Zotero key: `E6KDCS3V`
- 연도: `2019`
- 저자: Baumgartner, Manuel; Sagebaum, Max; Gauger, Nicolas R.; Spichtinger, Peter; Brinkmann, Andre
- DOI/URL: `10.5194/gmd-12-5197-2019`
- 컬렉션 섹션: `자동미분·미분가능 구현`
- 컬렉션 내 역할: KDM6AD의 민감도/AD 필요성을 뒷받침하는 직접 근거
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`102540` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD가 “미세물리 scheme 자동미분”이라는 학술 계보에 속한다는 가장 강한 근거 중 하나다.
- operator-overloading AD가 legacy atmospheric physics 코드에서 tape, memory, runtime overhead, validation 문제를 낳는다는 현실을 보여준다.
- parameter sensitivity 식별이 단순 코드기술이 아니라 cloud scheme 불확실성 과학으로 연결됨을 보여준다.
...
- 수동 TL/AD moist physics 전통에서 modern AD tool을 cloud scheme에 적용한 결절점이다.
- Hieronymus 2022, differentiable ESM, JAX-SCM/JCM으로 이어지는 differentiable atmospheric physics 계보의 앞부분이다.
- KDM6AD는 CoDiPack 대신 libtorch/C++ AD surface와 Fortran ABI를 사용한다는 구현상 차별점이 있다.
...
이 위치 때문에 Baumgartner 2019은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- cloud scheme 코드를 AD type으로 재컴파일하거나 감싸서 tangent/adjoint sensitivities를 얻는다.
- scheme output이 uncertain parameters에 얼마나 민감한지 분석한다.
- AD overhead와 numerical behavior를 실제 atmospheric physics code context에서 평가한다.
...
- F(y,theta) -> outputs에서 JVP dF/dtheta * delta theta와 VJP lambda^T dF/dtheta가 핵심이다.
- AD가 산출하는 것은 discrete code derivative이므로, branch/limiter가 포함된 실제 implementation derivative다.
- KDM6AD도 continuous equation derivative가 아니라 ported discrete operator derivative를 제공한다는 점을 명시해야 한다.
...
- KDM6AD의 방법론 section에서 직접 비교해야 한다. CoDiPack-source AD vs libtorch operator graph/ABI bridge 차이를 설명할 수 있다.
- parameter sensitivity와 자료동화 gradient를 모두 “cloud scheme AD의 용도”로 묶을 수 있다.
- AD correctness는 finite difference, tangent-adjoint consistency, physical sanity checks로 다층 검증해야 한다.
...
- cloud microphysics AD는 이미 독립 연구 계보를 가진 주제다.
- KDM6AD의 차별성은 KDM6 forward parity와 Fortran-host integration을 유지한 AD surface라는 점이다.
...
- IFS liquid/cloud scheme과 KDM6/WDM6 mixed-phase scheme은 물리 범위와 코드 구조가 다르다.
- AD tool이 다르므로 성능/메모리 결과를 KDM6AD에 직접 적용하면 안 된다.
...
| 축 | Baumgartner 2019에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Baumgartner 2019을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Baumgartner 2019을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Baumgartner 2019의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
...
- [[paper-L2BC8MGF|Tao 1989]] - An Ice-Water Saturation Adjustment
