...
이 페이지는 Lim 2022 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** ICE-POP 겨울폭풍 평가를 통해 WDM6/WDM7 및 bulk-type microphysics가 실제 winter storm hydrometeor properties를 얼마나 다르게 만드는지 보여준다.

## 기본 서지와 근거 상태

- Zotero key: `5T4INXZ3`
- 연도: `2022`
- 저자: Lim, Kyo-Sun Sunny; Kim, Kwonil; Ko, Jeong-Su; Lee, Gyuwon; Thompson, Gregory; Berne, Alexis
- DOI/URL: `10.5194/gmd-15-4529-2022`
- 컬렉션 섹션: `KDM/WDM 계열 핵심 미세물리`
- 컬렉션 내 역할: KDM/WDM6 계열의 과학적 계보와 forward parity 기준
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`114061` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD가 단순 이상화 실험이 아니라 winter precipitation 및 관측 검증으로 확장될 때 필요한 물리적 평가 문맥을 제공한다.
- snow/graupel/cloud ice representation이 radar/satellite-like observable과 연결되는 방식을 이해하게 한다.
- KDM6AD 자료동화 확장에서는 관측이 민감한 hydrometeor category가 무엇인지 판단하는 배경이 된다.
...
- WDM6/WDM7 evaluation lineage에서 field-campaign/observational comparison 쪽으로 이동한 문헌이다.
- Thompson winter precipitation, P3, riming/graupel-density 문헌과 함께 ice-phase forecast skill 계보를 이룬다.
- KDM6AD에서는 forward parity 이후 “어느 관측 레짐에서 gradient를 쓸 것인가”를 정하는 데 연결된다.
...
이 위치 때문에 Lim 2022은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- bulk microphysics schemes를 겨울폭풍 사례에 적용하고 hydrometeor distribution과 precipitation properties를 비교한다.
- ICE-POP 관측 맥락에서 snow/graupel/rain transition, vertical structure, forecast biases를 본다.
- scheme 간 차이를 총강수량뿐 아니라 microphysical pathway의 차이로 해석한다.
...
- winter mixed-phase에서는 melting/freezing/sublimation/deposition branch가 강하고, 이 branch들이 KDM6AD에서 piecewise-smooth gradient를 만든다.
- radar reflectivity는 PSD의 고차 moment에 민감해 q parity만으로는 관측공간 parity가 보장되지 않을 수 있다.
- JVP 실험 설계에서는 temperature perturbation과 ice-category perturbation을 분리해야 한다.
...
- 자료동화용 loss를 reflectivity 또는 precipitation type으로 잡을 때 어떤 hydrometeor gradient를 기대해야 하는지 알려준다.
- KDM6AD의 winter storm 적용 가능성을 논할 때 forward-only 평가문헌으로 쓸 수 있다.
- 관측 검증과 gradient correctness는 별도 문제임을 명시해야 한다.
...
- winter precipitation에서는 ice-phase category와 PSD closure가 forecast skill을 지배할 수 있다.
- KDM6AD 자료동화 확장은 rain-only가 아니라 snow/graupel-sensitive observation operator를 고려해야 한다.
...
- 이 논문이 KDM6AD 구현을 검증하지는 않는다.
- 관측 campaign case의 결론을 모든 계절/지역으로 확장하면 안 된다.
...
| 축 | Lim 2022에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Lim 2022을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Lim 2022을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Lim 2022의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
...
- [[paper-3FNR6KLT|Thompson 2008]] - Explicit Forecasts of Winter Precipitation Using an Improved Bulk Microphysics Sc…
