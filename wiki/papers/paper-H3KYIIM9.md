...
**핵심 판정:** WDM6가 실제 precipitating convection에서 어떤 예측 특성을 갖는지 평가해, KDM6AD가 forward parity 후 무엇을 물리적으로 보존해야 하는지 알려주는 검증 계보 문헌이다.

## 기본 서지와 근거 상태

- Zotero key: `H3KYIIM9`
- 연도: `2010`
- 저자: Lim, Kyo-Sun Sunny; Hong, Song-You; Lee, Yong-Hee; Ha, Jong-Chul; Kim, Hyung-Woo; Ham, Sook-Jeong; Dudhia, Jimy
- DOI/URL: `10.1155/2010/707253`
- 컬렉션 섹션: `KDM/WDM 계열 핵심 미세물리`
- 컬렉션 내 역할: KDM/WDM6 계열의 과학적 계보와 forward parity 기준
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`76142` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- 개발 논문이 아니라 평가 논문이라는 점이 중요하다. scheme의 가치가 상태변수 설계만으로 끝나지 않고 convective precipitation, radar/bright-band 구조, aerosol-sensitive response로 검증되어야 함을 보여준다.
- KDM6AD가 AD 기능을 추가하더라도, 먼저 원래 WDM/KDM forward behavior가 보존되어야 한다는 연구 설계의 근거가 된다.
- 이 논문은 KDM6AD 원고의 “strict parity before gradients” 스토리와 자연스럽게 연결된다.
...
- WDM6 개발 직후의 성능평가 결절점이다.
- 이후 WDM7 revision과 ICE-POP 겨울 강수 평가로 이어지는 평가 계보의 앞부분에 놓인다.
- 자료동화 문헌과 연결하면 reflectivity/precipitation observation이 미세물리 검증에서 왜 중요한지 설명할 수 있다.
...
- precipitating convection case에서 WDM6와 관련 scheme의 강수, cloud/rain number, vertical hydrometeor structure를 비교한다.
- 평가 초점은 단순 total precipitation이 아니라 convective/stratiform 분리, bright-band, droplet/rain size distribution 차이다.
- scheme의 수치 안정성과 물리 response를 operational-style model setting에서 확인한다.
...
- evaluation paper라도 수학적 핵심은 P_micro(y)의 출력이 관측가능한 precipitation/reflectivity proxy로 어떻게 사상되는지다.
- KDM6AD에서는 forward diagnostic D(y)와 AD state y의 경계를 분명히 해야 한다. 진단량이 loss에 들어가면 dD/dy가 필요하지만, parity만 볼 때는 diagnostic-only일 수 있다.
- case evaluation은 Jacobian의 정답을 주지 않지만, gradient가 의미 있게 해석되어야 할 physical regime을 알려준다.
...
- KDM6AD의 forward equivalence test를 논문 계보상에 위치시킨다.
- DA 관점에서는 “어떤 관측공간의 오차를 줄이고 싶은가”를 정해 주는 평가 배경이다.
- 강수 검증 결과만으로 AD correctness를 주장하지 말고, forward regime selection 근거로 써야 한다.
...
- WDM6 계열의 학술적 평가는 convective precipitation 구조와 hydrometeor distribution을 함께 봐야 한다.
- KDM6AD 연구에서는 forward parity와 gradient utility를 분리 검증해야 한다.
...
- 개별 case 평가의 결론을 모든 KDM6AD 적용 영역으로 일반화하면 안 된다.
- 수동/자동 adjoint 검증을 제공하지 않는다.
