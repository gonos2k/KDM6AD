...
이 페이지는 Lin 1983 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** bulk snow-field parameterization의 고전적 출발점으로, 현대 WSM/WDM/KDM 계열 frozen hydrometeor category의 역사적 바닥을 이룬다.

## 기본 서지와 근거 상태

- Zotero key: `66SUJCSY`
- 연도: `1983`
- 저자: Lin, Yuh-Lang; Farley, Richard D.; Orville, Harold D.
- DOI/URL: `10.1175/1520-0450(1983)022<1065:bpotsf>2.0.co;2`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`1456` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD 페이지에서 “bulk microphysics”의 뿌리를 설명할 때 단순 배경이 아니라 category-based snow/graupel parameterization의 원형으로 기능한다.
- single-moment category scheme이 왜 강력하면서도 제한적인지 보여준다.
- AD 관점에서는 오래된 bulk closure가 현대 differentiable implementation에서도 여전히 미분대상이 될 수 있음을 보여준다.
...
- Lin-Farley-Orville snow parameterization은 WSM6/KDM6 frozen-process 계보의 핵심 조상이다.
- Thompson, WSM/WDM, P3, graupel density 문헌은 모두 이 category-based 전통을 계승하거나 수정한다.
- KDM6AD가 현행 scheme을 미분가능하게 옮기는 작업은 이런 역사적 closure를 그대로 계산 그래프에 올리는 일이다.
...
이 위치 때문에 Lin 1983은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- snow field를 bulk category로 두고 deposition, aggregation, accretion, melting, sedimentation을 parameterized process로 닫는다.
- PSD assumption과 empirical fall speed/collection relation이 핵심 구현 지점이다.
- explicit bin-resolved microphysics가 아니라 computationally feasible한 bulk approximation을 선택한다.
...
- q_s와 assumed PSD parameter가 process rate를 결정한다. single-moment에서는 N_s가 독립 state가 아니므로 lambda 또는 intercept parameter가 prescribed/diagnostic이다.
- KDM6AD의 gradient는 이런 prescribed parameter에 대해 state sensitivity만 제공할 수 있으며, parameter sensitivity는 별도 제어변수로 열어야 한다.
- hard thresholds for phase and melting/freezing create non-smooth tangent points.
...
- forward parity 기준의 역사적 의미를 제공한다. KDM6AD는 고전 bulk closure를 새 물리로 바꾸지 않고 미분가능한 계산 경로로 복제해야 한다.
- parameter inference를 하려면 single-moment fixed parameters를 theta로 분리하는 설계가 필요하다.
- 자료동화에서는 snow-related reflectivity gradient가 PSD assumption에 강하게 의존함을 설명한다.
...
- KDM6AD의 frozen-process gradient는 고전 bulk snow parameterization 계보 위에 있다.
- single-moment closure의 parameter uncertainty는 AD-based calibration의 자연스러운 후보가 된다.
...
- 현대 KDM6의 모든 process 세부를 이 논문 하나로 설명할 수 없다.
- 고전 empirical relation을 학술적 최종 정답처럼 쓰면 안 된다.
...
| 축 | Lin 1983에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Lin 1983을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Lin 1983을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Lin 1983의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
