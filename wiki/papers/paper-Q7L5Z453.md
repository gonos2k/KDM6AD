...
이 페이지는 Morrison 2008 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** CAM3 two-moment scheme의 기후 response와 aerosol-cloud interaction을 다루어, microphysics closure가 climate feedback과 radiative forcing 해석에 연결됨을 보여준다.

## 기본 서지와 근거 상태

- Zotero key: `Q7L5Z453`
- 연도: `2008`
- 저자: Morrison, H.; Gettelman, A.; Ghan, S. J.
- DOI/URL: `10.1175/2008jcli2116.1`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`90630` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD가 단기 NWP/DA를 넘어서 differentiable climate-process 연구로 확장될 가능성을 보여주는 비교문헌이다.
- aerosol indirect effect와 cloud number sensitivity는 KDM6AD의 CCN/Nc gradient와 개념적으로 맞닿아 있다.
- 모델 스케일이 달라도 cloud microphysics derivative가 radiation/precipitation tendency로 전파된다는 점을 설명한다.
...
- Part I의 scheme description에서 Part II의 climate response로 이어진다.
- LIMA, WDM6 CCN, equation discovery cloud cover, hybrid AI climate model과 aerosol/cloud-climate 계보를 공유한다.
- KDM6AD의 자료동화보다는 differentiable parameter inference/closure calibration 측면에 더 가깝다.
...
이 위치 때문에 Morrison 2008은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- two-moment stratiform cloud scheme을 CAM3에 넣고 aerosol-cloud-climate response를 평가한다.
- cloud droplet number, precipitation efficiency, radiative properties가 coupling된 결과를 분석한다.
- single process rate가 climate statistics에 축적되는 방식을 보여준다.
...
- Nc perturbation은 autoconversion/accretion뿐 아니라 optical properties와 radiation tendency로 전파된다.
- loss가 TOA radiation이나 cloud cover라면 VJP는 radiation operator와 microphysics operator를 거쳐 number/mass state로 되돌아간다.
- KDM6AD에서는 현재 radiation coupling이 AD surface 밖이라면 이 경로를 주장하지 말아야 한다.
...
- KDM6AD의 CCN/Nc gradient가 장기적으로 radiation-aware differentiable modeling과 연결될 수 있음을 보여준다.
- hybrid AI cloud cover 문헌과 결합해 parameterization inference story를 만들 수 있다.
- 현행 KDM6AD의 scope가 microphysics parity라면 climate feedback 주장은 future work로 제한해야 한다.
...
- number concentration sensitivity는 precipitation뿐 아니라 cloud-radiation coupling까지 확장될 수 있다.
- KDM6AD의 장기 학술 가치는 differentiable process coupling으로 확장될 여지가 있다.
...
- CAM3 climate response를 KDM6AD/KIM host 성능으로 등치하면 안 된다.
- radiation coupling derivative는 현재 코드에 포함된 범위인지 별도 확인이 필요하다.
...
| 축 | Morrison 2008에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Morrison 2008을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Morrison 2008을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Morrison 2008의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
...
- [[paper-M9XUQ75C|Grundner 2025]] - Reduced Cloud Cover Errors in a Hybrid AI-Climate Model Through Equation Discover…
