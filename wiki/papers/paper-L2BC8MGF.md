...
이 페이지는 Tao 1989 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** ice-water saturation adjustment는 미세물리 scheme에서 빠르고 안정적인 phase-equilibrium 처리를 가능하게 하지만, AD 관점에서는 threshold와 projection이 만드는 비매끄러움의 핵심 사례다.

## 기본 서지와 근거 상태

- Zotero key: `L2BC8MGF`
- 연도: `1989`
- 저자: Tao, Wei-Kuo; Simpson, Joanne; McCumber, Michael
- DOI/URL: `10.1175/1520-0493(1989)117<0231:aiwsa>2.0.co;2`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`260` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD가 단순 source-term AD가 아니라 saturation adjustment와 limiter까지 다루어야 함을 보여주는 수치물리 문헌이다.
- 수치적으로는 안정성을 주지만, differentiability audit에서는 가장 주의해야 할 projection operator 계열이다.
- 자료동화에서는 moist adjoint의 오래된 난점, 즉 on/off process와 saturation constraint의 선형화 문제를 연결한다.
...
- bulk microphysics의 process-rate 계보와 별도로, thermodynamic adjustment/constraint projection 계보를 대표한다.
- WRFDA 4DVar와 AD cloud scheme 문헌에서 반복되는 moist threshold 문제의 선행 배경이다.
- KDM6AD에서는 forward parity를 깨지 않고 이 projection을 어떻게 AD surface에 노출할지가 핵심이다.
...
이 위치 때문에 Tao 1989은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- water/ice saturation을 기준으로 vapor, cloud water/ice, temperature를 조정한다.
- equilibrium constraint를 빠르게 만족시키기 위해 explicit source integration보다 diagnostic adjustment를 사용한다.
- phase selection은 temperature와 saturation state에 따라 branch된다.
...
- adjustment는 y -> Pi_C(y) 형태의 projection으로 볼 수 있다. C는 saturation manifold 또는 inequality constraint다.
- 활성 constraint가 바뀌는 지점에서 dPi_C/dy는 불연속이거나 정의가 약해진다.
- KDM6AD의 JVP/VJP는 branch 내부에서는 유효하지만 branch crossing perturbation에서는 해석을 제한해야 한다.
...
- KDM6AD Differentiability Audit에서 smooth/piecewise-smooth/numerically fragile 구분의 대표 사례다.
- DA에서 moist variables를 control로 둘 때 saturation adjustment adjoint가 observation increment를 어떻게 분배하는지 설명한다.
- gradient check는 branch-stable perturbation과 branch-crossing perturbation을 나누어 해야 한다.
...
- saturation adjustment는 미세물리 AD에서 가장 중요한 비매끄러운 연산자 가족이다.
- KDM6AD 검증은 forward parity와 branch-aware gradient 검사를 함께 가져야 한다.
...
- 이 논문은 KDM6의 전체 microphysics process를 설명하는 문헌이 아니다.
- AD 가능성을 주장할 때 smooth optimization처럼 과장하면 안 된다.
...
| 축 | Tao 1989에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Tao 1989을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Tao 1989을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Tao 1989의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
