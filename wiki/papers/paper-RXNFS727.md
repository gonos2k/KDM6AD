...
section: "미세물리 설계공간"
paper_use: "CCN/aerosol 및 수농도 민감도 축"
aliases:
...
이 페이지는 Mansell 2013 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** aerosol perturbation이 two-moment bulk microphysics를 통해 storm electrification과 precipitation에 미치는 효과를 분석해, aerosol-cloud-precipitation sensitivity의 storm-scale 경로를 보여준다.

## 기본 서지와 근거 상태

- Zotero key: `RXNFS727`
- 연도: `2013`
- 저자: Mansell, Edward R.; Ziegler, Conrad L.
- DOI/URL: `10.1175/jas-d-12-0264.1`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: CCN/aerosol 및 수농도 민감도 축
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`102334` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- WDM6의 prognostic CCN과 연결해 aerosol/CCN sensitivity가 단지 droplet number 변화가 아니라 storm dynamics와 precipitation/electrification으로 전파됨을 보여준다.
- KDM6AD에서 NCCN gradient를 해석할 때 “local activation derivative”와 “storm-scale response”를 분리해야 함을 알려준다.
- aerosol parameter/control variable을 DA 또는 parameter inference에 넣을 때 identifiability 문제가 큼을 시사한다.
...
- Mansell two-moment electrification 계열의 aerosol sensitivity 응용이다.
- Seifert/Beheng maritime vs continental CCN experiment와 WDM6 CCN prognostic state를 storm-scale comparison으로 이어 준다.
- KDM6AD는 이 계보의 differentiable operator block을 제공할 수 있다.
...
이 위치 때문에 Mansell 2013은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- aerosol loading 또는 CCN-related condition을 바꾸어 storm electrification/precipitation response를 비교한다.
- two-moment microphysics가 droplet spectrum, ice interactions, charge separation을 매개한다.
- storm-scale feedback을 통해 aerosol effect가 nonlinear하게 증폭/변형된다.
...
- NCCN perturbation delta c가 activation A(c,S)와 droplet number Nc를 거쳐 precipitation P와 charge Q로 전파된다.
- dP/dc는 단일 process derivative가 아니라 time-integrated coupled sensitivity다.
- KDM6AD local JVP는 이 chain의 microphysics block을 제공하지만 full dP/dc는 dynamics/electrification tangent가 필요하다.
...
- KDM6AD의 CCN-control DA 또는 parameter inference 가능성을 설명할 때 유용하다.
- VJP로 강수 오차가 CCN 축으로 돌아갈 수 있음을 계보적으로 정당화한다.
- 하지만 aerosol effect는 regime-dependent이므로 gradient sign을 일반화하면 안 된다.
...
- CCN/aerosol sensitivity는 WDM/KDM 계열 double-moment state에서 자연스럽게 발생한다.
- KDM6AD는 aerosol-cloud-precipitation sensitivity 연구의 계산 블록으로 위치할 수 있다.
...
- electrification coupling은 KDM6AD 현행 범위 밖이다.
- case-specific aerosol response를 보편 법칙으로 쓰면 안 된다.
...
| 축 | Mansell 2013에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Mansell 2013을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Mansell 2013을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Mansell 2013의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
...
- [[paper-PR3T9Z88|Seifert 2005]] - A two-moment cloud microphysics parameterization for mixed-phase clouds. Part 2: …
