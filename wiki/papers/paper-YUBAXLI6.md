...
**핵심 판정:** bulk cloud microphysics에 partial cloudiness를 도입해 grid cell 내부의 cloud fraction과 process rate 차이를 explicit하게 다루려는 KIM/홍 계열 설계 문헌이다.

## 기본 서지와 근거 상태

- Zotero key: `YUBAXLI6`
- 연도: `2018`
- 저자: Hong, Song-You; Kim, So-Young
- DOI/URL: `10.1175/jas-d-17-0234.1`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 격자 평균 과정률과 부분운량/아격자 변동성 이슈
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`48062` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM/KIM 계열과 직접 가까운 partial cloudiness 문헌으로, KDM6AD가 grid-mean operator만 미분할 때 놓칠 수 있는 subgrid closure 문제를 보여준다.
- Pincus 2000의 이론적 문제의식을 실제 bulk microphysics scheme design으로 가져온다.
- KDM6AD가 자료동화로 확장될 때 관측 footprint와 모델 cloud fraction mismatch를 설명하는 핵심 배경이다.
...
- Unresolved variability -> partial cloudiness concept -> precipitation process effects로 이어지는 계보의 중심이다.
- KIM system 및 KDM/WDM 계열 physics package 문맥과 연결된다.
- KDM6AD가 future KIM host DA로 가려면 이 partial cloudiness closure와 연결될 가능성이 크다.
...
- 격자 내 cloudy fraction과 clear fraction을 구분해 microphysical process rate 계산을 조정한다.
- 2D tests를 통해 partial cloudiness가 cloud/precipitation evolution에 미치는 영향을 평가한다.
- bulk scheme의 computational efficiency를 유지하면서 subgrid cloud heterogeneity를 반영한다.
...
- F_eff(y, C)=C F(y/C) 같은 형태의 effective cloudy-area process rate로 해석할 수 있다. 실제 식은 scheme별로 다르지만 핵심은 cloud fraction C가 nonlinear rate scaling에 들어간다는 점이다.
- JVP에는 delta C와 delta q가 서로 얽힌다. C가 threshold로 결정되면 branch sensitivity가 생긴다.
- KDM6AD가 C를 AD state로 포함하지 않으면 partial-cloud sensitivity는 제공하지 않는다.
...
- KDM6AD 자료동화에서 cloud fraction 또는 subgrid parameter를 control로 둘 수 있는지 묻는 후속 질문을 만든다.
- 현행 KDM6AD의 local microphysics gradient가 grid-mean closure derivative인지 cloudy-area derivative인지 명시해야 한다.
- 관측연산자와 cloud fraction coupling을 future work로 제시할 수 있다.
...
- subgrid cloud fraction은 bulk microphysics process rate와 gradient 해석을 바꿀 수 있다.
- KDM6AD의 DA 확장은 cloud fraction closure를 무시하면 관측공간에서 한계를 가질 수 있다.
...
- partial cloudiness 구현이 현행 KDM6AD parity 대상에 포함되었는지는 코드로 별도 확인해야 한다.
- 2D result를 모든 운영 예보로 일반화하면 안 된다.
