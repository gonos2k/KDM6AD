...
section: "미세물리 설계공간"
paper_use: "격자 평균 과정률과 부분운량/아격자 변동성 이슈"
aliases:
  - "Effects of Partial Cloudiness in a Cloud Microphysics Scheme on Simulated Precipitation Processes During a Boreal Summer"
  - "Bae 2019"
  - "YKPE6B2X"
---
# Effects of Partial Cloudiness in a Cloud Microphysics Scheme on Simulated Precipitation Processes During a Boreal Summer
...
이 페이지는 Bae 2019 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** partial cloudiness가 실제 simulated precipitation processes에 어떤 영향을 주는지 평가해, 개념 제안에서 예측 영향 분석으로 계보를 확장한다.

## 기본 서지와 근거 상태

- Zotero key: `YKPE6B2X`
- 연도: `2019`
- 저자: Bae, Soo Ya; Kim, So‐Young; Park, Rae‐Seol; Hong, Song‐You
- DOI/URL: `10.1029/2018jd029519`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 격자 평균 과정률과 부분운량/아격자 변동성 이슈
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`75312` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- YUBAXLI6가 concept/2D result라면 이 논문은 precipitation process 영향에 초점을 둔다.
- KDM6AD가 forward parity 후 precipitation-sensitive gradient를 해석할 때 subgrid closure가 결과를 바꿀 수 있음을 보여준다.
- Korea-region precipitation physics package 문맥에서 KDM6AD 설명을 더 현실적으로 만든다.
...
- Pincus unresolved variability와 Hong partial cloudiness concept의 후속 평가 문헌이다.
- KIM system paper, WDM6/KDM6 precipitation evaluation, ICE-POP evaluation과 연결된다.
- KDM6AD의 자료동화/관측 확장에서는 cloud fraction과 precipitation observation 사이의 계보적 다리다.
...
이 위치 때문에 Bae 2019은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- partial cloudiness scheme을 넣은 경우와 그렇지 않은 경우의 precipitation process 차이를 비교한다.
- cloud water/rain/snow/graupel production과 precipitation distribution 변화를 분석한다.
- bulk microphysics 내부 process가 precipitation forecast에 미치는 영향을 scheme modification 관점에서 평가한다.
...
- 비선형 process rate가 C-dependent scaling을 받으면 dP/dq와 dP/dC가 모두 중요해진다.
- precipitation loss의 VJP가 hydrometeor state뿐 아니라 cloud fraction parameter로 돌아갈 수 있음을 시사한다.
- KDM6AD 현행 state에 C가 없다면 이 sensitivity는 missing control dimension이다.

원문/매니페스트에서 확인한 수학적 읽기 단서:
- 원문 추출의 수식/기술 단서는 해당 논문을 process-rate closure 또는 model-coupling 문헌으로 읽게 한다.
- 원문 추출에서 PSD/gamma distribution 또는 moment-closure 관련 표현이 확인되어, size spectrum closure를 수학 축으로 읽는다.
...
- KDM6AD 자료동화 확장 때 control vector 설계에 cloud fraction/subgrid parameter를 포함할지 논의하게 한다.
- forward parity를 KDM6 core에만 맞춘 뒤, host-level partial cloud coupling을 어떻게 처리할지 분리해야 한다.
- 관측 기반 precipitation 개선 주장은 partial cloudiness closure 포함 여부를 명확히 해야 한다.
...
- precipitation process sensitivity는 subgrid cloud treatment에 의존한다.
- KDM6AD gradient 활용은 state vector 밖 closure parameter를 어떻게 다룰지까지 확장되어야 한다.
...
- KDM6AD 구현 검증 논문은 아니다.
- partial cloudiness 효과는 resolution, case, host physics에 의존한다.
...
| 축 | Bae 2019에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Bae 2019을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Bae 2019을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Bae 2019의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
...
- [[paper-S2R9LH3S|Pincus 2000]] - Unresolved spatial variability and microphysical process rates in large‐scale mod…
- [[paper-5T4INXZ3|Lim 2022]] - Simulated microphysical properties of winter storms from bulk-type microphysics s…
- [[paper-DFJTS2Q8|Hong 2018]] - The Korean Integrated Model (KIM) System for Global Weather Forecasting
