...
이 페이지는 Lim 2024 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** WRF WDM6에 graupel density prediction을 도입해 graupel을 고정밀도 범주가 아니라 진화하는 물리 성질로 다루려는 최신 계보 문헌이다.

## 기본 서지와 근거 상태

- Zotero key: `D629MKTV`
- 연도: `2024`
- 저자: Lim, Kyo-Sun Sunny; Kim, Kwonil; Lee, Gyuwon; Park, Sun-Young; Milbrandt, Jason A.
- DOI/URL: `10.5194/gmd-17-7199-2024`
- 컬렉션 섹션: `KDM/WDM 계열 핵심 미세물리`
- 컬렉션 내 역할: KDM/WDM6 계열의 과학적 계보와 forward parity 기준
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`104279` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD 계보에서 가장 직접적인 최신 frozen-hydrometeor refinement 문헌이다.
- graupel density가 fall speed, melting, reflectivity, precipitation distribution에 영향을 주므로 AD state 확장 후보를 구체화한다.
- 기존 KDM6AD가 엄격 forward parity를 우선한다면, 이 논문은 parity 후 extension story의 핵심 참고문헌이 된다.
...
- WDM6 -> WDM7 -> WDM6 graupel density prediction으로 이어지는 WDM 계열 후속 개선이다.
- Lin-Colle riming intensity 및 P3 particle-property prediction과 같은 문제의식, 즉 frozen particle property를 고정 category 너머로 확장하는 흐름에 놓인다.
- KDM6AD의 자료동화 확장에서는 radar reflectivity와 graupel property sensitivity를 연결하는 교량이다.
...
이 위치 때문에 Lim 2024은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- graupel density를 예측하거나 진단해 graupel fall speed와 process rate를 기존 고정값보다 물리적으로 가변화한다.
- WRF WDM6 구조 안에서 computationally manageable한 방식으로 frozen hydrometeor property를 추가한다.
- 평가에서는 convective precipitation, graupel distribution, reflectivity 또는 precipitation intensity 변화를 본다.
...
- rho_g가 state 또는 diagnostic function이면 v_t = v_t(rho_g,D), collection/melting terms P_g(y,rho_g)가 된다.
- JVP에는 delta rho_g -> delta fall speed -> delta sedimentation/precipitation 경로가 추가된다.
- VJP에서는 reflectivity/precipitation loss가 rho_g와 q_g 사이에서 어떻게 분배되는지가 새로운 해석문제다.
...
- KDM6AD의 ABI에 diag_rhog 같은 진단량을 넣을지, AD-packed state로 승격할지 판단하는 직접 선행문헌이다.
- 현행 코드가 `diag_rhog`를 forward diagnostic으로 다루는 경우, 이 논문은 그 diagnostic의 물리적 의미를 설명하는 데 유용하다.
- 자료동화 확장에서는 graupel density control 또는 weak-constraint parameter inference 가능성을 제시한다.
...
- graupel density는 fixed-category bulk scheme의 중요한 불확실성 축이다.
- KDM6AD의 장기 확장에서는 graupel property sensitivity가 radar/precipitation DA와 연결될 수 있다.
...
- 현행 KDM6AD가 이 논문의 모든 closure를 구현한다는 뜻은 아니다.
- density prediction을 AD state로 넣으면 parity, ABI, memory layout, gradient validation이 모두 바뀐다.
...
| 축 | Lim 2024에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Lim 2024을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Lim 2024을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Lim 2024의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
