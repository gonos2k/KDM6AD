...
이 페이지는 Morrison 2009 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** squall line trailing stratiform precipitation의 발달이 microphysics choice에 민감함을 보여주어, scheme-level process differences가 mesoscale organization으로 증폭되는 경로를 설명한다.

## 기본 서지와 근거 상태

- Zotero key: `VJK3VRCB`
- 연도: `2009`
- 저자: Morrison, H.; Thompson, G.; Tatarskii, V.
- DOI/URL: `10.1175/2008mwr2556.1`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`94689` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD가 미분하는 local microphysics source term이 단지 column budget이 아니라 storm organization과 precipitation structure에 영향을 줄 수 있음을 보여준다.
- WDM6 평가와 자료동화 문헌을 연결하는 mesoscale process bridge다.
- gradient interpretation에서 “local derivative”와 “dynamically coupled outcome”의 차이를 분명히 한다.
...
- Morrison double-moment 계열의 convective system 응용 문헌이다.
- Mansell electrification/aerosol effects와 함께 storm-scale two-moment microphysics lineage를 형성한다.
- KDM6AD를 DA에 쓰면 microphysics perturbation이 dynamics를 거쳐 precipitation organization으로 전파되는 chain이 문제된다.
...
이 위치 때문에 Morrison 2009은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- squall line simulation에서 microphysics scheme 선택이 trailing stratiform precipitation과 convective structure에 미치는 영향을 비교한다.
- hydrometeor distributions, fallout, latent heating, cold pool/downdraft response를 연결해 해석한다.
- one-moment vs two-moment 또는 process treatment 차이를 storm morphology에 연결한다.
...
- full forecast operator M = DYN ∘ MICRO ∘ ... 이므로 d precipitation / d microphysics state는 time-integrated tangent를 통해 축적된다.
- 단일 timestep KDM6AD JVP는 이 chain의 MICRO block을 제공하지만, storm-scale sensitivity는 host dynamics tangent까지 필요하다.
- VJP에서는 최종 강수 loss가 여러 timestep의 microphysics states로 되돌아가므로 checkpointing 문제가 생긴다.
...
- KDM6AD를 forecast-DA에 연결할 때 local microphysics AD만으로 충분하지 않음을 알려준다.
- microphysics-only gradient와 full-model gradient를 구분하는 논리적 근거가 된다.
- storm-scale 검증은 gradient utility를 평가하는 별도 단계다.
...
- microphysics scheme choice는 storm-scale precipitation structure에 증폭될 수 있다.
- KDM6AD의 MICRO-block AD는 full-model DA chain의 필요조건이지 충분조건은 아니다.
...
- 이 논문은 KDM6AD single-step gradient를 검증하지 않는다.
- mesoscale organization 결과를 microphysics operator 하나의 효과로만 단순화하면 안 된다.
...
| 축 | Morrison 2009에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Morrison 2009을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Morrison 2009을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Morrison 2009의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
