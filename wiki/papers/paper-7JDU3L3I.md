...
**핵심 판정:** CAM3에 two-moment stratiform cloud microphysics를 도입한 description/numerical test 문헌으로, climate-model bulk microphysics에서 double-moment state가 어떤 의미를 갖는지 보여준다.

## 기본 서지와 근거 상태

- Zotero key: `7JDU3L3I`
- 연도: `2008`
- 저자: Morrison, Hugh; Gettelman, Andrew
- DOI/URL: `10.1175/2008jcli2105.1`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`101975` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- WDM6가 mesoscale/WRF 문맥이라면, 이 논문은 climate-model stratiform 문맥의 double-moment 설계다.
- KDM6AD가 column/process operator로 추상화될 때 모델 스케일이 달라도 moment closure의 수학은 공유됨을 보여준다.
- numerical test 중심의 Part I은 process-level sanity check의 중요성을 강조한다.
...
- Morrison double-moment 계열에서 CAM implementation으로 이어지는 축이다.
- Part II aerosol-cloud-climate response와 함께 KDM6AD의 aerosol/CCN gradient 해석 배경을 제공한다.
- JAX-SCM, hybrid AI cloud cover 문헌과 climate/SCM differentiable model 계보로 연결된다.
...
- stratiform cloud microphysics에 mass와 number moment를 도입하고 numerical tests로 scheme behavior를 평가한다.
- climate model grid에서 cloud droplet/ice number와 precipitation processes의 영향을 본다.
- process decomposition과 numerical stability가 중요한 초점이다.
...
- double-moment state y=(q,N)에서 process operator F_micro는 time-stepper와 coupling되어 climate tendency를 만든다.
- SCM/column model에서 KDM6AD-like JVP는 local process sensitivity를 격자 평균 tendency 민감도로 해석할 수 있다.
- time integration stability와 limiter가 differentiability에 영향을 준다.
...
- KDM6AD를 single-column differentiable experiment로 확장할 때 비교 배경이 된다.
- q,N moment의 climate-scale sensitivity와 mesoscale sensitivity가 다르게 해석되어야 함을 알려준다.
- Part I은 forward numerical tests, KDM6AD는 parity+AD tests라는 검증 구조를 비교할 수 있다.
...
- double-moment microphysics는 weather model뿐 아니라 climate model physics에서도 핵심 설계축이다.
- KDM6AD의 process-level AD는 SCM/column 실험으로 학술적 의미를 확장할 수 있다.
...
- CAM3 stratiform scheme과 KDM6 scheme은 구현과 물리 범위가 다르다.
- climate-scale conclusion을 KIM mesoscale host에 직접 적용하면 안 된다.
