...
**핵심 판정:** Seifert-Beheng Part 1은 mixed-phase two-moment microphysics의 model description으로, mass와 number density를 함께 예측하는 comprehensive scheme의 핵심 선행문헌이다.
...
- DOI/URL: `10.1007/s00703-005-0112-4`
- 컬렉션 섹션: `원문 보강 필요`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: 원문 PDF 미확보. Springer 공개 Summary 기준: https://link.springer.com/article/10.1007/s00703-005-0112-4

## 학술적 가치

- WDM6가 selective double-moment extension이라면, 이 논문은 cloud droplets, raindrops, cloud ice, snow, graupel 전반에 mass/number를 예측하는 더 포괄적 two-moment 계보다.
- CCN activation, nucleation, collision/coalescence, breakup, collection kernels를 함께 다루어 KDM6AD의 process-operator 해석을 풍부하게 한다.
- 원문 미확보 상태에서도 공개 초록이 scheme 범위를 명확히 알려주는 핵심 문헌이다.
...
- Seifert/Beheng warm-rain double-moment와 mixed-phase extension에서 WDM/LIMA/P3 비교축으로 이어진다.
- Part 2 maritime/continental CCN sensitivity와 쌍을 이룬다.
- KDM6AD의 WDM6 CCN/Nc/Nr state는 이 넓은 two-moment 철학의 operationally efficient subset으로 읽을 수 있다.
...
- cloud droplets, raindrops, cloud ice, snow, graupel의 mass와 number density evolution을 parameterize한다.
- CCN activation과 여러 homogeneous/heterogeneous nucleation process를 포함한다.
- coalescence turbulence effect, raindrop breakup, size-dependent collision efficiencies, collection kernel approximation을 다룬다.
...
- 각 hydrometeor x에 대해 (q_x,N_x) moment pair가 있고, process rate는 collision kernel K(D_i,D_j)의 moment integral로 닫힌다.
- KDM6AD에서는 WDM6보다 적은 number variables만 미분하므로, full two-moment mixed-phase Jacobian과 다른 sparsity를 가진다.
- kernel approximation과 collision integral은 differentiability와 computational cost의 주요 지점이다.
...
- KDM6AD의 process 목록을 학술적으로 설명할 때 “전 범주 two-moment”와 “WDM6의 selective two-moment” 차이를 명확히 한다.
- CCN activation derivative와 number concentration derivative의 물리적 계보를 제공한다.
- 원문 미확보라 세부 계수와 검증 결과는 사용하지 않는다.
...
- mixed-phase two-moment scheme은 mass와 number density를 함께 예측해 aerosol/precipitation effects를 더 명시적으로 다룬다.
- KDM6AD의 selective double-moment AD는 broader two-moment mixed-phase 계보 안에 위치한다.
...
- 현재 확보 근거는 Springer 공개 summary와 메타데이터다.
- KDM6AD가 Seifert-Beheng scheme을 구현한다고 쓰면 안 된다.
