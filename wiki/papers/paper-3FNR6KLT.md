...
이 페이지는 Thompson 2008 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** winter precipitation forecast를 개선하기 위한 Thompson 계열 bulk scheme 문헌으로, snow/ice particle representation과 cold-season verification의 비교축을 제공한다.

## 기본 서지와 근거 상태

- Zotero key: `3FNR6KLT`
- 연도: `2008`
- 저자: Thompson, Gregory; Field, Paul R.; Rasmussen, Roy M.; Hall, William D.
- DOI/URL: `10.1175/2008mwr2387.1`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 미세물리 설계공간 또는 비교 문헌
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`110009` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- KDM6AD의 KDM/WDM 계보와 독립적인 operational bulk microphysics 계열을 비교할 수 있게 한다.
- winter precipitation, supercooled liquid water, snow PSD, riming 같은 문제가 KDM6AD의 ice-phase gradient 해석과 연결된다.
- ICE-POP 및 P3 문헌과 함께 frozen hydrometeor 설계공간을 넓힌다.
...
- Lin snow-field 전통을 개선한 winter precipitation bulk scheme lineage다.
- P3와 graupel-density 문헌처럼 ice particle property 문제를 다루지만 category/closure 전략은 다르다.
- KDM6AD가 특정 WDM/KDM scheme에 한정된다는 점을 비교문헌을 통해 선명하게 한다.
...
이 위치 때문에 Thompson 2008은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- winter precipitation case에서 snow/ice/graupel processes와 fall-speed/PSD assumptions를 조정한다.
- observationally relevant precipitation type과 vertical hydrometeor structure를 평가한다.
- bulk scheme의 cold-cloud parameterization을 operational forecast 관점에서 검증한다.
...
- ice/snow PSD와 fall-speed moment가 precipitation flux를 결정한다.
- temperature-dependent phase branch와 saturation branch가 JVP/VJP에서 민감한 구간을 만든다.
- KDM6AD의 ice sensitivity가 WDM/KDM closure에 국한된다는 점을 비교할 때 유용하다.
...
- winter storm DA로 확장할 때 Thompson/P3/WDM 계열 간 gradient 의미 차이를 설명한다.
- KDM6AD 결과를 타 scheme보다 우월하다고 주장하지 말고, scheme-specific differentiable operator로 위치시켜야 한다.
- cold-season branch-aware finite-difference gradient test의 필요성을 보여준다.
...
- winter precipitation microphysics는 category, PSD, fall-speed closure에 매우 민감하다.
- KDM6AD의 frozen-process gradient 해석에는 타 bulk scheme과의 계보적 비교가 필요하다.
...
- KDM6/KDM6AD 직접 조상은 아니다.
- scheme intercomparison 결과를 KDM6AD 코드 검증으로 사용할 수 없다.
...
| 축           | Thompson 2008에서 확인할 내용                                                                  | KDM6AD에서 남는 의미                                                          |
| ----------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
...
이 매트릭스의 목적은 Thompson 2008을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Thompson 2008을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Thompson 2008의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
