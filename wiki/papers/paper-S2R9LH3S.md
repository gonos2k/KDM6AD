...
이 페이지는 Pincus 2000 논문을 일반 초록 수준으로 요약하지 않고, [[KDM6]], [[KDM6AD]], [[KDM6 Literature Genealogy]], [[KDM6AD Literature Claim Map]] 안에서 어떤 학술적 결절점으로 쓰이는지를 정리한다. 목표는 논문 작성용 카드가 아니라, KDM6AD 연구의 과학사·수학·구현·자료동화 계보에서 이 논문이 담당하는 역할을 분명히 하는 것이다.

**핵심 판정:** 격자 평균 상태로 비선형 미세물리 process rate를 계산할 때 subgrid variability를 무시하면 체계적인 rate bias가 생긴다는 문제를 정식화한다.

## 기본 서지와 근거 상태

- Zotero key: `S2R9LH3S`
- 연도: `2000`
- 저자: Pincus, Robert; Klein, Stephen A.
- DOI/URL: `10.1029/2000jd900504`
- 컬렉션 섹션: `미세물리 설계공간`
- 컬렉션 내 역할: 격자 평균 과정률과 부분운량/아격자 변동성 이슈
- 근거 상태: PDF 원문 및 추출 텍스트 확보 (`78373` chars extracted). 원문 기반으로 구조를 정리했다.

## 학술적 가치

- partial cloudiness와 subgrid cloud fraction이 왜 bulk microphysics에서 중요한지 수학적으로 설명하는 배경이다.
- KDM6AD에서 미분하는 F(y)가 grid-mean state에 대한 local operator인지, subgrid distribution을 평균한 effective operator인지 구분하게 만든다.
- 자료동화에서 관측 footprint와 모델 격자 평균이 다를 때 gradient 해석이 왜 조심스러워야 하는지 알려준다.
...
- large-scale model subgrid variability 논의에서 Hong/Bae partial cloudiness microphysics로 이어진다.
- CAM3 two-moment scheme과 climate-model cloud process 계보에도 연결된다.
- KDM/KIM host의 bulk scheme이 grid-mean closure를 쓸 때 그 이론적 한계를 드러낸다.
...
이 위치 때문에 Pincus 2000은/는 [[KDM6 Literature Genealogy]]에서 단순 참고문헌이 아니라 선행 가정, 비교 계보, 구현 방법론, 또는 관측 응용 중 하나의 노드로 다루어야 한다. 특히 KDM6AD를 설명할 때는 “새 물리 scheme”이라는 표현보다 “기존 KDM/WDM 및 관련 bulk microphysics 계보의 discrete operator를 AD 가능한 표면으로 노출한다”는 해석이 더 정확하다.

## 방법론과 모형 구조

- process rate R(q)에서 E[R(q)]와 R(E[q])가 다르다는 Jensen-type 문제를 다룬다.
- cloud fraction, subgrid PDFs, unresolved variability가 autoconversion/accretion 등 nonlinear process에 주는 영향을 분석한다.
- bulk microphysics를 격자 평균 모델에 넣을 때 필요한 scale-awareness를 제시한다.
...
- R이 convex/thresholded이면 평균 상태에서 계산한 rate는 subgrid 평균 rate와 달라진다.
- AD에서 dR(E[q])/dq는 실제 E[dR/dq]를 대표하지 않을 수 있다. 이것은 gradient interpretation의 scale error다.
- partial cloudiness는 effective operator F_eff(y,alpha_cloud)를 만들며 alpha_cloud에 대한 sensitivity가 추가된다.
...
- KDM6AD의 JVP/VJP가 local-column operator의 민감도임을 명확히 할 때 필요하다.
- 자료동화에서는 관측이 subgrid cloud object를 보는데 모델이 grid-mean state를 미분하는 불일치를 설명한다.
- partial cloudiness parameters를 calibration target으로 둘 수 있다는 연구질문을 만든다.
...
- 비선형 미세물리 process의 grid-mean closure는 구조적 bias를 가질 수 있다.
- KDM6AD gradient는 operator scale과 closure assumption을 함께 명시해야 학술적으로 의미가 있다.
...
- KDM6AD 현행 구현에 subgrid PDF closure가 포함되었다는 뜻은 아니다.
- 이 논문 하나로 모든 partial cloud scheme 세부를 설명할 수 없다.
...
| 축 | Pincus 2000에서 확인할 내용 | KDM6AD에서 남는 의미 |
| --- | --- | --- |
...
이 매트릭스의 목적은 Pincus 2000을/를 단순히 “관련 논문”으로 묶지 않고, KDM6AD의 물리 계보에서 어떤 추상화 수준에 놓이는지 고정하는 것이다. 같은 bulk microphysics 문헌이라도 하나는 직접 조상, 하나는 대안 closure, 하나는 구현 방법론, 하나는 관측연산자 계보일 수 있다. 이 구분이 없으면 KDM6AD 설명은 문헌 목록은 길지만 과학적 주장은 흐린 상태가 된다.
...
Pincus 2000을/를 KDM6AD 코드와 연결할 때는 다음 대응을 확인한다.
...
이 분해는 실제 코드의 정확한 call graph가 아니라 학술적 독해를 위한 operator decomposition이다. 중요한 점은 KDM6AD가 미분하는 대상이 “구름물리 일반”이 아니라 특정 closure와 branch를 가진 discrete KDM6 계열 연산자라는 사실이다. 따라서 Pincus 2000의 가치는 KDM6AD의 모든 주장을 보증하는 데 있지 않고, 이 discrete operator가 어떤 계보적 선택 위에 서 있는지 밝히는 데 있다.
