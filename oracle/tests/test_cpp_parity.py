"""C++ kdm6_fn  ↔  Python _kdm6_pure  numeric parity.
...
PRECISION HONESTY: the C++ dumps print via std::hexfloat
...
on the current KDM6AD-k artifact is comparing the f32 Fortran-faithful C++ mirror against the
Python f64 oracle. The 3-sub-cycle CPPOUT path therefore carries expected f32-level roundoff
(currently O(1e-8) relative), while the assertion below remains a regression bound that catches
orders-of-magnitude one-sided drift.

The ICs below MUST stay in lockstep with the dump blocks in
libtorch/tests/test_autograd_endtoend.cpp (CPPOUT=g_base dt=300, RCEOUT dt=120,
...
_BIN = _REPO / "libtorch" / "build" / "test_autograd_endtoend"
...
def test_cpp_python_forward_parity():
    """C++ kdm6_fn ↔ Python _kdm6_pure stay within the f32 mirror regression bound."""
    ...
    # Regression bound: current C++ is the f32 Fortran-faithful mirror; a real one-sided
    # bug jumps relΔ by orders of magnitude beyond the observed O(1e-8) CPPOUT drift.
    ...
    assert worst_rel < 1e-6, f"parity regressed — {msg}"
