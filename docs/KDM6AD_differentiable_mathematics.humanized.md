...
- **작동 경로 (float32)**: `torch::NoGradGuard` 아래에서 실행되어 autograd 테이프를 남기지 않고,
...
두 경로의 분기는 데이터에 의존하는 `.item()` 조건이 아니라 **`scalar_type()` 구조적 검사**로 갈리므로
...
`value_only=false`(DA 호출)는 테이프를 살린 채 forward를 실행한 뒤, 그래프의 두 끝점(입력 리프·출력)을
`Handle`에 담아 이후 VJP/JVP에 쓴다.
...
합성 사상 $F$는 다음 스위칭 곡면들이 유도하는 분할의 각 셀 **내부에서 $C^\infty$**이고, 경계에서만
Lipschitz(꺾임)이거나 불연속이다 (`coordinator.py` 전역 취합):
...
- **연속·꺾임(Lipschitz)형**: $D$(ReLU 클램프 $\text{clamp}(\cdot,0)$ — 경계에서 값 연속, 기울기만 꺾임),
  ...
  그리고 전이 rate가 게이트 경계에서 $\to0$으로 사라지는 rate-형(대부분의 warm/cold 항). 이런 곳에서는 곱셈
  where-마스크가 한쪽 편도함수(Clarke 부분기울기의 한 원소)를 사실상 제공한다.
...
소사이클 사이에 detach가 없으므로 입력 리프 → 출력까지가 **하나의 연속 autograd 그래프**로 이어진다.
...
그래프는 오로지 `state_in` 리프와 `state_out` 텐서를 `Handle::Impl` 안에 **살려두는 것만으로** 유지된다.
...
입력에 대해 역전파하면 정확히 $J^\top u$가 나온다. 바로 이 점이 구현의 핵심이다.
...
`grad_outputs={}`는 스칼라에 암묵적으로 $1.0$을 주며, 실제 seed는 $s=\langle F(x),u\rangle$에 이미 담겨 있다.
...
**상대오차 $<10^{-12}$**로 검증된다 — 이중-VJP JVP가 VJP 수반의 정확한 전치임을 증명한다. 양변이
모두 역방향이라 FD에 얽매이지 않으며, 비매끄러운 방향(24성분 전체)까지 포함한다.
...
전용 `hvp()`/`hessian()` 함수는 따로 없다. HVP 역시 JVP를 구동하는 바로 그 **동일한 이중-역전파 원시연산**으로 얻는다. 스칼라 손실 $L(x)=\phi(F(x))$에 대해 $g(x)=\nabla_x L$이고,

$$\nabla_x\big(g(x)^\top v\big)=\nabla^2_x L(x)\,v=(\text{HVP}).$$

`GraphOptions.create_graph` 플래그(`runtime.h:84`, "true → grad-of-grad (double-VJP)")가 겉으로 드러난 스위치다:
...
`vjp(create_graph=true)`가 반환한 $g=J^\top u$는 그 자체로 그래프의 노드다. 따라서 $g$로 만든 스칼라를 다시 `torch::autograd::grad`하면 grad-of-grad, 즉 스칼라 손실의 헤시안-벡터 곱이 나온다.
...
이 검증은 헤시안-벡터형 이중 역전파에 해당하며, 2차 도함수가 유한·비영이어야 함을 요구한다. 검증 대상 커스텀 Function은
`RgmmaT`/`LibmLog`/`LibmExp`/`LibmPowTensor`로, 각 backward가 미분가능한 torch 연산만 쓴다(§7).
...
두 가지 이유가 맞물린다 (`ops.cpp:12-14, 57-59`):
...
비트 정확한 대응이 된다. **순수 텐서 연산이라 커스텀 autograd 규칙이 필요 없고 InferenceMode에서도 안전**하다.
...
전미분으로 보면 $t\sqrt t=t^{3/2}$는 $t=0$에서 도함수 $\tfrac32\sqrt t$가 **유한(=0)**하다. 반면 autograd는 곱셈규칙을
그대로 적용한다:

$$\frac{d}{dt}\big[t\cdot\sqrt t\big]=\sqrt t+t\cdot\frac{1}{2\sqrt t}.$$

둘째 항에서는 $t=0$이 $\sqrt{\cdot}$ 노드를 거치는데, 그 국소 기울기 $1/(2\sqrt t)\to+\infty$가 $0\cdot\infty=\text{NaN}$을
만든다. 4D-Var 제어변수가 온위 $\theta$일 때 $t=\theta\,\Pi$가 일시적으로 $\le0$이 될 수 있으므로 이 문제는 **실제로 발화**한다.
해법은 $\text{clamp}(t,\min=1\,\mathrm{K})$다. 이렇게 하면 sqrt 노드 기울기가 $1/(2\sqrt1)=0.5$로 유한해지고, 물리 온도(>1K) 범위에서는 아무 영향도 없다.
...
운반하고, 이산 `mstep`은 상수로만 전달된다. detach의 세 용도: (1) 작동 forward의 NoGradGuard(그래프 0),
...
그래서 명제 3·4는 게이트/클램프 꺾임을 피하려고 **매끄러운 열역학 방향** $(\theta,q_v,q_c)$로 제한한다(S4-jump류 회피).
...
DA 진입점은 `double*` 패킹을 따로 받아 모든 텐서를 `ScalarType::Double`로 올리고 12개 상태장에
...
자체를 해제**한다. 따라서 닫은 뒤 같은 포인터를 재사용하면 정의되지 않은 동작(UB)이다
...
각 부연산자의 변환 대상과 비매끄러운 점을 아래에 정리한다. **권위 기준은 실제 이식본이자 Fortran 원본과 비트-일치하는
...
per-cell 게이트(`coordinator.py:1586` vs `coordinator.cpp:2561`). 아래의 줄 참조는 이해를 돕기 위한 오라클 위치이고,
정합성 기준값은 C++을 따른다.
...
KDM6AD의 미분가능성은 세 축이 맞물려 성립한다: **(1)** dtype을 스위치로 삼아 하나의 연산자 코드가 f32 비트-동일
...
$\langle Jv,u\rangle=\langle v,J^\top u\rangle$이 상대오차 $10^{-12}$로 성립한다는 점이 이 구성 전체의 정확성을 증명하며,
