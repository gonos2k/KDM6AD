...
- **⑰ P1-1 보존형·bounded CVT — partition 스테이지** (2026-07-12, 커밋
  ...
  합성해 총수분이 구성상 불변이고 증분이 배경-동결 cap으로 유계
  (무한 exp 제거 — P0-3 근본원인). **liq↔ice 직접 채널 없음**: vap↔liq(−d)
  + vap↔ice(+d) 합성의 순 잠열이 정확히 L_f = xls−xl(T) — §37 분기-조건
  잠열상수가 CVT에서 완전히 배제 (판정 패널의 단순화 채택, w=0 kink 없음).
  ...
  정규화) 구성으로 회복성 재확립. 전체 스위트 699 통과. **v1
