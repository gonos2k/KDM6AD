...
- **⑱ 보존형 CVT 증거 artifact 합격 — v10 conserving_stress 실행**
  ...
  5.55e-17** (5,494 컬럼 × 39층 컬럼적분 총수분, 문자 그대로 1 ulp —
  구성상 보존의 실증) vs 의도된 qv 대각 총수분 변화 max 1.56e-3 /
  mean 4.3e-5 kg/kg (리뷰 명령의 분리 기록); ② **J 1,119,818→1,080,111
  (−3.55%)**, |O−B| 10.270→9.934K — v9.2 (−3.63%, 9.926K) 대비 제한된
  제어공간(비보존 mul → 보존 채널)의 비용이 J 0.08%p / O−A 0.008K에
  불과, **OMA≤OMB 게이트 재보정 불필요**; ③ 채널 활동: n_active
  ...
  Δq=−1.1e-9 (구조적 불변), 맑음/구름 22.729→22.208K (+6.7e-6 생성 —
  ...
  **리뷰 정정 (artifact 승인·종결 유지, 표현·강제성 후속)**: ① v9.2
  ...
  conserving_contract 게이트 신설 (partition schema v2 + 질량 수상체
  n_controlled 전부 0); negative 회귀 매트릭스 17종. **아카이브된 v10
