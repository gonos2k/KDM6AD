...
- **⑰ P1-1 보존형·bounded CVT — partition 스테이지** (2026-07-12, 커밋
  9e97e6f + 914c1f6 + 본 커밋). 2단 디코더 `x0 = P_w(D_v(xb))`: 대각 CVT
  ...
  검증). 전체 스위트 692 통과. **v1 한계(명시)**: 단일-패스 고정 순서의
  표현력 갭(한 디코드 내 rain→vapor 불가), 생성은 파티션 밖(pseudo-RH),
  full-domain LC05 보존형 증거 실행은 후속 (제한된 제어공간에서
  OMA≤OMB 게이트 재보정 필요 여부 포함).
