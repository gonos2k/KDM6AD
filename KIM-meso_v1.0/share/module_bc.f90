...
wrf_err_message )
   ...
   SUBROUTINE flow_dep_bdy_qnn  (  field,          &
      ...
      INTEGER,      INTENT(IN   )    :: its,ite, jts,jte, kts,kte
      INTEGER,      INTENT(IN   )    :: spec_zone
      REAL,         INTENT(IN   )    :: ccn_conc, scale_h 
      ...
      REAL,  DIMENSION( ims:ime , kms:kme ), INTENT(IN   ) :: xland
      TYPE( grid_config_rec_type ) config_flags
          ...
          DO k = kts, ktf
            DO i = max(its,b_limit+ibs), min(itf,ibe-b_limit)
              ...
              IF(v(i,k,j) .lt. 0.)THEN
                field(i,k,j) = field(i_inner,k,jbs+spec_zone)
              ELSE
                z_sum = z_sum + dz8w(i,j,k)
                if(xland(i,j)==1) then
                  field(i,k,j) = (5000.*exp(-0.4*z_sum/1000.)+100.)*1.e6
                else
                  field(i,k,j) = (150.*exp(-0.35*z_sum/1000.)+10.)*1.e6
                endif
          ...
          DO k = kts, ktf
            DO i = max(its,b_limit+ibs), min(itf,ibe-b_limit)
              ...
              ELSE
                z_sum = z_sum + dz8w(i,j,k)
                if(xland(i,j)==1) then
                  field(i,k,j) = ccn_conc*50.*exp(-z_sum/scale_h)
                else
                  field(i,k,j) = ccn_conc*0.5*exp(-z_sum/scale_h)
               endif
