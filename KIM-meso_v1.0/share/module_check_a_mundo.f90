  ...
  enddo
   ...
   END IF
     ...
     IF ( model_config_rec%charnock_update .EQ. 0 ) THEN 
          model_config_rec%io_form_auxinput6 = 0
          DO i = 1, model_config_rec % max_dom
            IF ( .NOT. model_config_rec % grid_allowed(i) ) CYCLE
                wrf_err_message = '--- NOTE: charnock_update is 0, ' // & 
                   'setting io_form_auxinput24 = 0 and auxinput6_interval = 0 for all domains'
            CALL wrf_debug ( 1, TRIM( wrf_err_message ) )
            model_config_rec%auxinput24_interval(i)   = 0  
            model_config_rec%auxinput24_interval_y(i) = 0
            model_config_rec%auxinput24_interval_d(i) = 0
            model_config_rec%auxinput24_interval_h(i) = 0
            model_config_rec%auxinput24_interval_m(i) = 0
            model_config_rec%auxinput24_interval_s(i) = 0
          ENDDO
      ELSE 
        IF ( model_config_rec%io_form_auxinput24 .EQ. 0 ) THEN 
            wrf_err_message = '--- ERROR: If charnock_update /= 0, io_form_auxinput5 must be /= 0'
            CALL wrf_message ( wrf_err_message )
            wrf_err_message = '--- Set io_form_auxinput24 in the time_control namelist (probably to 2).'
            CALL wrf_debug ( 0, TRIM( wrf_err_message ) )
            count_fatal_error = count_fatal_error + 1
         END IF
       END IF
       ...
       IF ( model_config_rec%charnock_update .EQ. 1 ) THEN 
         IF ( model_config_rec%io_form_auxinput24 .EQ. 0 ) THEN 
             wrf_err_message = '--- ERROR: If charnock_update /= 0, io_form_auxinput6 must be /= 0'
             CALL wrf_debug ( 0, TRIM(wrf_err_message) )
             wrf_err_message = '--- Set io_form_auxinput24 in the time_control namelist (probably to 2).'
             CALL wrf_debug ( 0, TRIM( wrf_err_message ) )
             count_fatal_error = count_fatal_error + 1
         END IF
         IF ( ( model_config_rec%auxinput24_interval(1)   .EQ. 0 ) .AND. &
              ( model_config_rec%auxinput24_interval_y(1) .EQ. 0 ) .AND. &
              ( model_config_rec%auxinput24_interval_d(1) .EQ. 0 ) .AND. &
              ( model_config_rec%auxinput24_interval_h(1) .EQ. 0 ) .AND. &
              ( model_config_rec%auxinput24_interval_m(1) .EQ. 0 ) .AND. &
              ( model_config_rec%auxinput24_interval_s(1) .EQ. 0 ) ) THEN 
             wrf_err_message = '--- ERROR: If charnock_update /= 0, one of the auxinput24_interval settings must be /= 0'
             CALL wrf_debug ( 0, TRIM(wrf_err_message) )
             wrf_err_message = '--- Set auxinput5_interval_s to the same value as interval_seconds (usually a pretty good guess).'
             CALL wrf_debug ( 0, TRIM( wrf_err_message ) )
             count_fatal_error = count_fatal_error + 1
          END IF
        END IF
