  ...
  ips, ipe, jps, jpe, kps, kpe )
   ...
   DO ij = 1, grid%num_tiles
     ...
     CALL phy_init (  grid%id , config_flags, grid%DT, grid%RESTART, grid%znw, grid%znu,   &
                      ...
                      grid%charnock,grid%u80,grid%v80,grid%u140,grid%v140,grid%u220,grid%v220,     & 
              ...
              grid%lhveg, grid%lhtran, & 
                      ...
                      ,grid%ccn_conc                                          & 
                      ,grid%scale_h                                           & 
                      ,grid%ncmin_land, grid%ncmin_sea                        & 
                      ,grid%QKE                                               &
...
CALL HALO_EM_HYDRO_NOAHMP_INIT_sub ( grid, &
  ...
  ips, ipe, jps, jpe, kps, kpe )
   ...
   IF ( f_qnn ) THEN
      IF ( config_flags%mp_physics == wdm5scheme .or. config_flags%mp_physics == wdm6scheme &
          .or. config_flags%mp_physics == kdm6scheme) THEN
         ...
         DO j=jts,MIN(jte,jde-1)
           DO i=its,MIN(ite,ide-1)
              ...
              DO k=kts,kte
                 ...
                 if(grid%xland(i,j)==1) then
                    scalar(i,k,j,p_qnn) = (5000.* exp(-0.4*z_sum/1000.)+100.)*1.e6
                 else
                    scalar(i,k,j,p_qnn) = (150.*exp(-0.35*z_sum/1000.)+10.)*1.e6
                 endif
