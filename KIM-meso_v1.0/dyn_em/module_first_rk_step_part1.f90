...
CALL HALO_EM_HYDRO_NOAHMP_sub ( grid, &
  ...
  ENDIF
      ...
      CALL nl_get_isoilwater(grid%id, isoilwater)           
...
&ifsnow,ISFFLX=config_flags%isfflx,CHS_jun=grid%CHS_jun,zol_jun=grid%zol_jun,br_jun=grid%br_jun,wspd_jun=grid%wspd_jun,ust_jun=grid&
&%ust_jun,hc_jun=grid%hc_jun,lai_jun=grid%lai_jun,lc_jun=grid%lc_jun,beta_jun=grid%beta_jun,zpd_jun=grid%zpd_jun,chs_ori=grid%chs_o&
&ri,FRACTIONAL_SEAICE=config_flags%fractional_seaice,SEAICE_ALBEDO_OPT=config_flags%seaice_albedo_opt,SEAICE_ALBEDO_DEFAULT=config_&
&flags%seaice_albedo_default,SEAICE_THICKNESS_OPT=config_flags%seaice_thickness_opt,SEAICE_THICKNESS_DEFAULT=config_flags%seaice_th&
&ickness_default,SEAICE_SNOWDEPTH_OPT=config_flags%seaice_snowdepth_opt,SEAICE_SNOWDEPTH_MAX=config_flags%seaice_snowdepth_max,SEAI&
&CE_SNOWDEPTH_MIN=config_flags%seaice_snowdepth_min,TICE2TSK_IF2COLD=config_flags%tice2tsk_if2cold,IFNDALBSI=grid%ifndalbsi,IFNDICE&
&DEPTH=grid%ifndicedepth,IFNDSNOWSI=grid%ifndsnowsi,DO_BIOE=do_bioe,DO_MEGANFILE=do_meganfile,ISLTYP=grid%isltyp,ITIMESTEP=grid%iti&
&mestep,JULIAN_IN=grid%julian,IRRIGATION=grid%irrigation,SF_SURF_IRR_SCHEME=config_flags%sf_surf_irr_scheme,IRR_DAILY_AMOUNT=config&
&_flags%irr_daily_amount,IRR_START_HOUR=config_flags%irr_start_hour,IRR_NUM_HOURS=config_flags%irr_num_hours,IRR_START_JULIANDAY=co&
&nfig_flags%irr_start_julianday,IRR_END_JULIANDAY=config_flags%irr_end_julianday,IRR_FREQ=config_flags%irr_freq,IRR_PH=config_flags&
&%irr_ph,IRR_RAND_FIELD=grid%irr_rand_field,IVGTYP=grid%ivgtyp,LH=grid%lh,LOWLYR=grid%lowlyr,MAVAIL=grid%mavail,NUM_SOIL_LAYERS=con&
&fig_flags%num_soil_layers,P8W=grid%p_hyd_w,PBLH=grid%pblh,PI_PHY=pi_phy,PSFC=grid%psfc,PSHLTR=grid%pshltr,PSIH=psih,BLDT=grid%bldt&
&,CURR_SECS=curr_secs,ADAPT_STEP_FLAG=adapt_step_flag,BLDTACTTIME=grid%bldtacttime,PSIM=psim,P_PHY=grid%p_hyd,Q10=grid%q10,Q2=grid%&
&q2,QFX=grid%qfx,QSFC=grid%qsfc,QSHLTR=grid%qshltr,QZ0=grid%qz0,RAINCV=grid%raincv,RA_LW_PHYSICS=config_flags%ra_lw_physics,RHO=gri&
&d%rho,RMOL=grid%rmol,SFCEVP=grid%sfcevp,SFCEXC=grid%sfcexc,SFCRUNOFF=grid%sfcrunoff,ACRUNOFF=grid%ACRUNOFF,opt_thcnd=config_flags%&
&opt_thcnd,SF_SFCLAY_PHYSICS=config_flags%sf_sfclay_physics,BL_PBL_PHYSICS=config_flags%bl_pbl_physics,SF_SURFACE_PHYSICS=config_fl&
&ags%sf_surface_physics,SH2O=grid%sh2o,SHDMAX=grid%shdmax,SHDMIN=grid%shdmin,SMOIS=grid%smois,SHDAVG=grid%shdavg,SMSTAV=grid%smstav&
...
&grid%sst_input,SST_UPDATE=grid%sst_update,SSTSK=grid%sstsk,DTW=grid%dtw,SST_SKIN=grid%sst_skin,CHARNOCK=grid%charnock,CHARNOCK_UPD&
&ATE=grid%charnock_update,U80=grid%u80,V80=grid%v80,U140=grid%u140,V140=grid%v140,U220=grid%u220,V220=grid%v220,SCM_FORCE_SKINTEMP=&
&grid%scm_force_skintemp,SCM_FORCE_FLUX=grid%scm_force_flux,STEPBL=grid%stepbl,TH10=grid%th10,TH2=grid%th2,THZ0=grid%thz0,TH_PHY=th&
&_phy,TKE_PBL=grid%tke_pbl,TMN=grid%tmn,TSHLTR=grid%tshltr,TSK=grid%tsk,TYR=grid%tyr,TYRA=grid%tyra,TDLY=grid%tdly,TLAG=grid%tlag,L&
&AGDAY=config_flags%lagday,NYEAR=grid%nyear,NDAY=grid%nday,TMN_UPDATE=grid%tmn_update,YR=yr,TSLB=grid%tslb,T_PHY=t_phy,U10=grid%u10&
&,URATX=grid%uratx,VRATX=grid%vratx,TRATX=grid%tratx,UDRUNOFF=grid%udrunoff,UST=grid%ust,UZ0=grid%uz0,U_FRAME=grid%u_frame,U_PHY=gr&
&id%u_phy,V10=grid%v10,U10E=grid%u10e,V10E=grid%v10e,UOCE=grid%uoce,VOCE=grid%voce,VEGFRA=grid%vegfra,VZ0=grid%vz0,V_FRAME=grid%v_f&
&rame,V_PHY=grid%v_phy,WARM_RAIN=grid%warm_rain,WSPD=grid%wspd,XICE=grid%xice,XLAND=grid%xland,MAX_EDOM=grid%num_ext_model_couple_d&
&om,CPLMASK=grid%cplmask,Z0=grid%z0,Z=grid%z,ZNT=grid%znt,ZS=grid%zs,ALBSI=grid%albsi,ICEDEPTH=grid%icedepth,SNOWSI=grid%snowsi,XIC&
&EM=grid%xicem,ISICE=grid%landuse_isice,USTM=grid%ustm,CK=grid%ck,CKA=grid%cka,CD=grid%cd,CDA=grid%cda,ISFTCFLX=config_flags%isftcf&
&lx,IZ0TLND=config_flags%iz0tlnd,SF_OCEAN_PHYSICS=config_flags%sf_ocean_physics,OML_HML0=config_flags%oml_hml0,OML_GAMMA=config_fla&
&gs%oml_gamma,TML=grid%tml,T0ML=grid%t0ml,HML=grid%hml,H0ML=grid%h0ml,HUML=grid%huml,HVML=grid%hvml,F=grid%f,TMOML=grid%TMOML,ISWAT&
&ER=iswater,OML_RELAXATION_TIME=grid%OML_RELAXATION_TIME,shalwater_z0=config_flags%shalwater_z0,water_depth=grid%water_depth,shalwa&
&ter_depth=config_flags%shalwater_depth,lakedepth2d=grid%lakedepth2d,savedtke12d=grid%savedtke12d,snowdp2d=grid%snowdp2d,h2osno2d=g&
&rid%h2osno2d,snl2d=grid%snl2d,t_grnd2d=grid%t_grnd2d,t_lake3d=grid%t_lake3d,lake_icefrac3d=grid%lake_icefrac3d,z_lake3d=grid%z_lak&
&e3d,dz_lake3d=grid%dz_lake3d,t_soisno3d=grid%t_soisno3d,h2osoi_ice3d=grid%h2osoi_ice3d,h2osoi_liq3d=grid%h2osoi_liq3d,h2osoi_vol3d&
&=grid%h2osoi_vol3d,z3d=grid%z3d,dz3d=grid%dz3d,zi3d=grid%zi3d,watsat3d=grid%watsat3d,csol3d=grid%csol3d,tkmg3d=grid%tkmg3d,tkdry3d&
&=grid%tkdry3d,tksatu3d=grid%tksatu3d,LakeModel=grid%sf_lake_physics,lake_min_elev=grid%lake_min_elev,LakeMask=grid%LakeMask,restar&
&t_flag=restart_flag,NUMC=grid%numc,NUMP=grid%nump,SABV=grid%sabv,SABG=grid%sabg,LWUP=grid%lwup,SNL=grid%snl,HISTORY_INTERVAL=confi&
&g_flags%history_interval,SNOWDP=grid%snowdp,WTC=grid%wtc,WTP=grid%wtp,H2OSNO=grid%h2osno,T_GRND=grid%t_grnd,T_VEG=grid%t_veg,H2OCA&
&N=grid%h2ocan,H2OCAN_COL=grid%h2ocan_col,T2M_MAX=grid%t2m_max,T2M_MIN=grid%t2m_min,T2CLM=grid%t2clm,T_REF2M=grid%t_ref2m,H2OSOI_LI&
&Q_S1=grid%h2osoi_liq_s1,H2OSOI_LIQ_S2=grid%h2osoi_liq_s2,H2OSOI_LIQ_S3=grid%h2osoi_liq_s3,H2OSOI_LIQ_S4=grid%h2osoi_liq_s4,H2OSOI_&
&LIQ_S5=grid%h2osoi_liq_s5,H2OSOI_LIQ1=grid%h2osoi_liq1,H2OSOI_LIQ2=grid%h2osoi_liq2,H2OSOI_LIQ3=grid%h2osoi_liq3,H2OSOI_LIQ4=grid%&
&h2osoi_liq4,H2OSOI_LIQ5=grid%h2osoi_liq5,H2OSOI_LIQ6=grid%h2osoi_liq6,H2OSOI_LIQ7=grid%h2osoi_liq7,H2OSOI_LIQ8=grid%h2osoi_liq8,H2&
&OSOI_LIQ9=grid%h2osoi_liq9,H2OSOI_LIQ10=grid%h2osoi_liq10,H2OSOI_ICE_S1=grid%h2osoi_ice_s1,H2OSOI_ICE_S2=grid%h2osoi_ice_s2,H2OSOI&
&_ICE_S3=grid%h2osoi_ice_s3,H2OSOI_ICE_S4=grid%h2osoi_ice_s4,H2OSOI_ICE_S5=grid%h2osoi_ice_s5,H2OSOI_ICE1=grid%h2osoi_ice1,H2OSOI_I&
&CE2=grid%h2osoi_ice2,H2OSOI_ICE3=grid%h2osoi_ice3,H2OSOI_ICE4=grid%h2osoi_ice4,H2OSOI_ICE5=grid%h2osoi_ice5,H2OSOI_ICE6=grid%h2oso&
&i_ice6,H2OSOI_ICE7=grid%h2osoi_ice7,H2OSOI_ICE8=grid%h2osoi_ice8,H2OSOI_ICE9=grid%h2osoi_ice9,H2OSOI_ICE10=grid%h2osoi_ice10,T_SOI&
&SNO_S1=grid%t_soisno_s1,T_SOISNO_S2=grid%t_soisno_s2,T_SOISNO_S3=grid%t_soisno_s3,T_SOISNO_S4=grid%t_soisno_s4,T_SOISNO_S5=grid%t_&
&soisno_s5,T_SOISNO1=grid%t_soisno1,T_SOISNO2=grid%t_soisno2,T_SOISNO3=grid%t_soisno3,T_SOISNO4=grid%t_soisno4,T_SOISNO5=grid%t_soi&
&sno5,T_SOISNO6=grid%t_soisno6,T_SOISNO7=grid%t_soisno7,T_SOISNO8=grid%t_soisno8,T_SOISNO9=grid%t_soisno9,T_SOISNO10=grid%t_soisno1&
&0,DZSNOW1=grid%dzsnow1,DZSNOW2=grid%dzsnow2,DZSNOW3=grid%dzsnow3,DZSNOW4=grid%dzsnow4,DZSNOW5=grid%dzsnow5,SNOWRDS1=grid%snowrds1,&
&SNOWRDS2=grid%snowrds2,SNOWRDS3=grid%snowrds3,SNOWRDS4=grid%snowrds4,SNOWRDS5=grid%snowrds5,T_LAKE1=grid%t_lake1,T_LAKE2=grid%t_la&
&ke2,T_LAKE3=grid%t_lake3,T_LAKE4=grid%t_lake4,T_LAKE5=grid%t_lake5,T_LAKE6=grid%t_lake6,T_LAKE7=grid%t_lake7,T_LAKE8=grid%t_lake8,&
&T_LAKE9=grid%t_lake9,T_LAKE10=grid%t_lake10,H2OSOI_VOL1=grid%h2osoi_vol1,H2OSOI_VOL2=grid%h2osoi_vol2,H2OSOI_VOL3=grid%h2osoi_vol3&
&,H2OSOI_VOL4=grid%h2osoi_vol4,H2OSOI_VOL5=grid%h2osoi_vol5,H2OSOI_VOL6=grid%h2osoi_vol6,H2OSOI_VOL7=grid%h2osoi_vol7,H2OSOI_VOL8=g&
&rid%h2osoi_vol8,H2OSOI_VOL9=grid%h2osoi_vol9,H2OSOI_VOL10=grid%h2osoi_vol10,MAXPATCH=config_flags%maxpatch,INEST=grid%id,ALBEDOsub&
&grid=grid%ALBEDOsubgrid,LHsubgrid=grid%LHsubgrid,HFXsubgrid=grid%HFXsubgrid,LWUPsubgrid=grid%LWUPsubgrid,Q2subgrid=grid%Q2subgrid,&
&SABVsubgrid=grid%SABVsubgrid,SABGsubgrid=grid%SABGsubgrid,NRAsubgrid=grid%NRAsubgrid,SWUPsubgrid=grid%SWUPsubgrid,LHsoi=grid%LHsoi&
&,LHveg=grid%LHveg,LHtran=grid%LHtran,t_veg24=grid%t_veg24,t_veg240=grid%t_veg240,fsun24=grid%fsun24,fsun240=grid%fsun240,fsd24=gri&
&d%fsd24,fsd240=grid%fsd240,fsi24=grid%fsi24,fsi240=grid%fsi240,laip=grid%laip,pct_pft_input=grid%pct_pft_input,num_pft_input=grid%&
&num_pft_clm,input_pft_flag=config_flags%input_pft,SLOPE_RAD=config_flags%slope_rad,TOPO_SHADING=config_flags%topo_shading,SHADOWMA&
&SK=grid%shadowmask,DIFFUSE_FRAC=grid%diffuse_frac,SLOPE=grid%slope,SLP_AZI=grid%slp_azi,SWNORM=grid%swnorm,DECLIN=grid%declin,SOLC&
&ON=grid%solcon,COSZEN=grid%coszen,HRANG=grid%hrang,xlat_urb2d=grid%XLAT,NUM_ROOF_LAYERS=num_roof_layers,NUM_WALL_LAYERS=num_wall_l&
&ayers,NUM_ROAD_LAYERS=num_road_layers,DZR=grid%dzr,DZB=grid%dzb,DZG=grid%dzg,TR_URB2D=grid%tr_urb2d,TB_URB2D=grid%tb_urb2d,TG_URB2&
&D=grid%tg_urb2d,TC_URB2D=grid%tc_urb2d,QC_URB2D=grid%qc_urb2d,UC_URB2D=grid%uc_urb2d,XXXR_URB2D=grid%xxxr_urb2d,XXXB_URB2D=grid%xx&
&xb_urb2d,XXXG_URB2D=grid%xxxg_urb2d,XXXC_URB2D=grid%xxxc_urb2d,CMCR_URB2D=grid%cmcr_urb2d,TGR_URB2D=grid%tgr_urb2d,TGRL_URB3D=grid&
&%tgrl_urb3d,SMR_URB3D=grid%smr_urb3d,JULIAN=grid%julday,JULYR=grid%julyr,DRELR_URB2D=grid%drelr_urb2d,DRELB_URB2D=grid%drelb_urb2d&
&,DRELG_URB2D=grid%drelg_urb2d,FLXHUMR_URB2D=grid%flxhumr_urb2d,FLXHUMB_URB2D=grid%flxhumb_urb2d,FLXHUMG_URB2D=grid%flxhumg_urb2d,T&
&RL_URB3D=grid%trl_urb3d,TBL_URB3D=grid%tbl_urb3d,TGL_URB3D=grid%tgl_urb3d,SH_URB2D=grid%sh_urb2d,LH_URB2D=grid%lh_urb2d,G_URB2D=gr&
&id%g_urb2d,RN_URB2D=grid%rn_urb2d,TS_URB2D=grid%ts_urb2d,FRC_URB2D=grid%frc_urb2d,UTYPE_URB2D=grid%utype_urb2d,SWDDIR=grid%swddir,&
&SWDDIF=grid%swddif,SF_URBAN_PHYSICS=config_flags%sf_urban_physics,num_urban_ndm=config_flags%num_urban_ndm,urban_map_zrd=config_fl&
&ags%urban_map_zrd,urban_map_zwd=config_flags%urban_map_zwd,urban_map_gd=config_flags%urban_map_gd,urban_map_zd=config_flags%urban_&
&map_zd,urban_map_zdf=config_flags%urban_map_zdf,urban_map_bd=config_flags%urban_map_bd,urban_map_wd=config_flags%urban_map_wd,urba&
&n_map_gbd=config_flags%urban_map_gbd,urban_map_fbd=config_flags%urban_map_fbd,urban_map_zgrd=config_flags%urban_map_zgrd,NUM_URBAN&
&_HI=config_flags%num_urban_hi,use_wudapt_lcz=config_flags%use_wudapt_lcz,slucm_distributed_drag=config_flags%slucm_distributed_dra&
&g,TSK_RURAL=grid%tsk_rural,TRB_URB4D=grid%trb_urb4d,TW1_URB4D=grid%tw1_urb4d,TW2_URB4D=grid%tw2_urb4d,TGB_URB4D=grid%tgb_urb4d,TLE&
&V_URB3D=grid%tlev_urb3d,QLEV_URB3D=grid%qlev_urb3d,TW1LEV_URB3D=grid%tw1lev_urb3d,TW2LEV_URB3D=grid%tw2lev_urb3d,TGLEV_URB3D=grid%&
&tglev_urb3d,TFLEV_URB3D=grid%tflev_urb3d,SF_AC_URB3D=grid%sf_ac_urb3d,LF_AC_URB3D=grid%lf_ac_urb3d,CM_AC_URB3D=grid%cm_ac_urb3d,SF&
&VENT_URB3D=grid%sfvent_urb3d,LFVENT_URB3D=grid%lfvent_urb3d,SFWIN1_URB3D=grid%sfwin1_urb3d,SFWIN2_URB3D=grid%sfwin2_urb3d,SFW1_URB&
&3D=grid%sfw1_urb3d,SFW2_URB3D=grid%sfw2_urb3d,SFR_URB3D=grid%sfr_urb3d,SFG_URB3D=grid%sfg_urb3d,EP_PV_URB3D=grid%ep_pv_urb3d,T_PV_&
&URB3D=grid%t_pv_urb3d,TRV_URB4D=grid%trv_urb4d,QR_URB4D=grid%qr_urb4d,QGR_URB3D=grid%qgr_urb3d,TGR_URB3D=grid%tgr_urb3d,DRAIN_URB4&
&D=grid%drain_urb4d,DRAINGR_URB3D=grid%draingr_urb3d,SFRV_URB3D=grid%sfrv_urb3d,LFRV_URB3D=grid%lfrv_urb3d,DGR_URB3D=grid%dgr_urb3d&
&,DG_URB3D=grid%dg_urb3d,LFR_URB3D=grid%lfr_urb3d,LFG_URB3D=grid%lfg_urb3d,LP_URB2D=grid%lp_urb2d,HI_URB2D=grid%hi_urb2d,LB_URB2D=g&
&rid%lb_urb2d,HGT_URB2D=grid%hgt_urb2d,MH_URB2D=grid%mh_urb2d,STDH_URB2D=grid%stdh_urb2d,LF_URB2D=grid%lf_urb2d,lf_urb2d_s=grid%lf_&
&urb2d_s,z0_urb2d=grid%z0_urb2d,GMT=grid%gmt,XLAT=grid%xlat,XLONG=grid%xlong,JULDAY=grid%julday,distributed_ahe_opt=grid%distribute&
&d_ahe_opt,ahe=grid%ahe,A_U_BEP=grid%a_u_bep,A_V_BEP=grid%a_v_bep,A_T_BEP=grid%a_t_bep,A_Q_BEP=grid%a_q_bep,B_U_BEP=grid%b_u_bep,B_&
&V_BEP=grid%b_v_bep,B_T_BEP=grid%b_t_bep,B_Q_BEP=grid%b_q_bep,SF_BEP=grid%sf_bep,VL_BEP=grid%vl_bep,A_E_BEP=grid%a_e_bep,B_E_BEP=gr&
&id%b_e_bep,DLG_BEP=grid%dlg_bep,DL_U_BEP=grid%dl_u_bep,CMR_SFCDIF=grid%cmr_sfcdif,CHR_SFCDIF=grid%chr_sfcdif,CMC_SFCDIF=grid%cmc_s&
&fcdif,CHC_SFCDIF=grid%chc_sfcdif,CMGR_SFCDIF=grid%cmgr_sfcdif,CHGR_SFCDIF=grid%chgr_sfcdif,LANDUSEF=grid%landusef,SOILCTOP=grid%so&
&ilctop,SOILCBOT=grid%soilcbot,RA=grid%ra,RS=grid%rs,LAI=grid%lai,IMPERV=grid%imperv,CANFRA=grid%canfra,NLCAT=grid%num_land_cat,NSC&
&AT=grid%num_soil_cat,VEGF_PX=grid%vegf_px,SNOWNCV=grid%snowncv,ANAL_INTERVAL=config_flags%auxinput9_interval_s+config_flags%auxinp&
&ut9_interval_m*60,PXLSM_SMOIS_INIT=config_flags%pxlsm_smois_init,PXLSM_SOIL_NUDGE=config_flags%pxlsm_soil_nudge,alswvisdir=grid%al&
&swvisdir,alswvisdif=grid%alswvisdif,alswnirdir=grid%alswnirdir,alswnirdif=grid%alswnirdif,swvisdir=grid%swvisdir,swvisdif=grid%swv&
&isdif,swnirdir=grid%swnirdir,swnirdif=grid%swnirdif,ssib_br=grid%ssib_br,ssib_fm=grid%ssib_fm,ssib_fh=grid%ssib_fh,ssib_cm=grid%ss&
&ib_cm,ssibxdd=grid%ssibxdd,ssib_lhf=grid%ssib_lhf,ssib_shf=grid%ssib_shf,ssib_ghf=grid%ssib_ghf,ssib_egs=grid%ssib_egs,ssib_eci=gr&
&id%ssib_eci,ssib_ect=grid%ssib_ect,ssib_egi=grid%ssib_egi,ssib_egt=grid%ssib_egt,ssib_sdn=grid%ssib_sdn,ssib_sup=grid%ssib_sup,ssi&
&b_ldn=grid%ssib_ldn,ssib_lup=grid%ssib_lup,ssib_wat=grid%ssib_wat,ssib_shc=grid%ssib_shc,ssib_shg=grid%ssib_shg,ssib_lai=grid%ssib&
&_lai,ssib_vcf=grid%ssib_vcf,ssib_z00=grid%ssib_z00,ssib_veg=grid%ssib_veg,cldfra=grid%cldfra,ISNOW=grid%isnow,SWE=grid%swe,SNOWDEN&
&=grid%snowden,SNOWDEPTH=grid%snowdepth,TKAIR=grid%tkair,DZO1=grid%dzo1,WO1=grid%wo1,TSSN1=grid%tssn1,TSSNO1=grid%tssno1,BWO1=grid%&
&bwo1,BTO1=grid%bto1,CTO1=grid%cto1,FIO1=grid%fio1,FLO1=grid%flo1,BIO1=grid%bio1,BLO1=grid%blo1,HO1=grid%ho1,DZO2=grid%dzo2,WO2=gri&
&d%wo2,TSSN2=grid%tssn2,TSSNO2=grid%tssno2,BWO2=grid%bwo2,BTO2=grid%bto2,CTO2=grid%cto2,FIO2=grid%fio2,FLO2=grid%flo2,BIO2=grid%bio&
&2,BLO2=grid%blo2,HO2=grid%ho2,DZO3=grid%dzo3,WO3=grid%wo3,TSSN3=grid%tssn3,TSSNO3=grid%tssno3,BWO3=grid%bwo3,BTO3=grid%bto3,CTO3=g&
&rid%cto3,FIO3=grid%fio3,FLO3=grid%flo3,BIO3=grid%bio3,BLO3=grid%blo3,HO3=grid%ho3,DZO4=grid%dzo4,WO4=grid%wo4,TSSN4=grid%tssn4,TSS&
&NO4=grid%tssno4,BWO4=grid%bwo4,BTO4=grid%bto4,CTO4=grid%cto4,FIO4=grid%fio4,FLO4=grid%flo4,BIO4=grid%bio4,BLO4=grid%blo4,HO4=grid%&
&ho4,RA_SW_PHYSICS=config_flags%ra_sw_physics,t2_ndg_old=grid%t2_ndg_old,q2_ndg_old=grid%q2_ndg_old,t2_ndg_new=grid%t2_ndg_new,q2_n&
&dg_new=grid%q2_ndg_new,sn_ndg_old=grid%sn_ndg_old,sn_ndg_new=grid%sn_ndg_new,pxlsm_modis_veg=config_flags%pxlsm_modis_veg,LAI_PX=g&
&rid%lai_px,WWLT_PX=grid%wwlt_px,WFC_PX=grid%wfc_px,WSAT_PX=grid%wsat_px,CLAY_PX=grid%clay_px,CSAND_PX=grid%csand_px,FMSAND_PX=grid&
&%fmsand_px,idveg=config_flags%dveg,iopt_crs=config_flags%opt_crs,iopt_btr=config_flags%opt_btr,iopt_run=config_flags%opt_run,iopt_&
&sfc=config_flags%opt_sfc,iopt_frz=config_flags%opt_frz,iopt_inf=config_flags%opt_inf,iopt_rad=config_flags%opt_rad,iopt_alb=config&
&_flags%opt_alb,iopt_snf=config_flags%opt_snf,iopt_tbot=config_flags%opt_tbot,iopt_stc=config_flags%opt_stc,iopt_gla=config_flags%o&
&pt_gla,iopt_rsf=config_flags%opt_rsf,iopt_soil=config_flags%opt_soil,iopt_pedo=config_flags%opt_pedo,iopt_crop=config_flags%opt_cr&
&op,iopt_irr=config_flags%opt_irr,iopt_irrm=config_flags%opt_irrm,iopt_infdv=config_flags%opt_infdv,iopt_tdrn=config_flags%opt_tdrn&
&,soiltstep=config_flags%soiltstep,isnowxy=grid%isnowxy,tvxy=grid%tvxy,tgxy=grid%tgxy,canicexy=grid%canicexy,canliqxy=grid%canliqxy&
&,eahxy=grid%eahxy,tahxy=grid%tahxy,cmxy=grid%cmxy,chxy=grid%chxy,fwetxy=grid%fwetxy,sneqvoxy=grid%sneqvoxy,alboldxy=grid%alboldxy,&
&qsnowxy=grid%qsnowxy,qrainxy=grid%qrainxy,wslakexy=grid%wslakexy,zwtxy=grid%zwtxy,waxy=grid%waxy,wtxy=grid%wtxy,tsnoxy=grid%tsnoxy&
&,zsnsoxy=grid%zsnsoxy,snicexy=grid%snicexy,snliqxy=grid%snliqxy,lfmassxy=grid%lfmassxy,rtmassxy=grid%rtmassxy,stmassxy=grid%stmass&
&xy,woodxy=grid%woodxy,stblcpxy=grid%stblcpxy,fastcpxy=grid%fastcpxy,grainxy=grid%grainxy,gddxy=grid%gddxy,pgsxy=grid%pgsxy,cropcat&
&=grid%cropcat,planting=grid%planting,harvest=grid%harvest,season_gdd=grid%season_gdd,soilcomp=grid%soilcomp,soilcl1=grid%soilcl1,s&
&oilcl2=grid%soilcl2,soilcl3=grid%soilcl3,soilcl4=grid%soilcl4,xsaixy=grid%xsaixy,taussxy=grid%taussxy,t2mvxy=grid%t2mvxy,t2mbxy=gr&
&id%t2mbxy,q2mvxy=grid%q2mvxy,q2mbxy=grid%q2mbxy,tradxy=grid%tradxy,neexy=grid%neexy,gppxy=grid%gppxy,nppxy=grid%nppxy,fvegxy=grid%&
&fvegxy,runsfxy=grid%runsfxy,runsbxy=grid%runsbxy,ecanxy=grid%ecanxy,edirxy=grid%edirxy,etranxy=grid%etranxy,fsaxy=grid%fsaxy,firax&
&y=grid%firaxy,aparxy=grid%aparxy,psnxy=grid%psnxy,savxy=grid%savxy,sagxy=grid%sagxy,rssunxy=grid%rssunxy,rsshaxy=grid%rsshaxy,bgap&
&xy=grid%bgapxy,wgapxy=grid%wgapxy,tgvxy=grid%tgvxy,tgbxy=grid%tgbxy,chvxy=grid%chvxy,chbxy=grid%chbxy,shgxy=grid%shgxy,shcxy=grid%&
&shcxy,shbxy=grid%shbxy,evgxy=grid%evgxy,evbxy=grid%evbxy,ghvxy=grid%ghvxy,ghbxy=grid%ghbxy,irgxy=grid%irgxy,ircxy=grid%ircxy,irbxy&
&=grid%irbxy,trxy=grid%trxy,evcxy=grid%evcxy,chleafxy=grid%chleafxy,chucxy=grid%chucxy,chv2xy=grid%chv2xy,chb2xy=grid%chb2xy,chstar&
&xy=grid%chstarxy,qintsxy=grid%qintsxy,qintrxy=grid%qintrxy,qdripsxy=grid%qdripsxy,qdriprxy=grid%qdriprxy,qthrosxy=grid%qthrosxy,qt&
&hrorxy=grid%qthrorxy,qsnsubxy=grid%qsnsubxy,qsnfroxy=grid%qsnfroxy,qsubcxy=grid%qsubcxy,qfrocxy=grid%qfrocxy,qfrzcxy=grid%qfrzcxy,&
&qmeltcxy=grid%qmeltcxy,qsnbotxy=grid%qsnbotxy,pondingxy=grid%pondingxy,PAHXY=grid%PAHXY,PAHGXY=grid%PAHGXY,PAHVXY=grid%PAHVXY,PAHB&
&XY=grid%PAHBXY,qmeltxy=grid%qmeltxy,FPICEXY=grid%FPICEXY,qevacxy=grid%qevacxy,qdewcxy=grid%qdewcxy,RAINLSM=grid%RAINLSM,SNOWLSM=gr&
&id%SNOWLSM,forctlsm=grid%forctlsm,forcqlsm=grid%forcqlsm,forcplsm=grid%forcplsm,forczlsm=grid%forczlsm,forcwlsm=grid%forcwlsm,acc_&
&ssoil=grid%acc_ssoil,acc_qinsur=grid%acc_qinsur,acc_qseva=grid%acc_qseva,acc_etrani=grid%acc_etrani,eflxbxy=grid%eflxbxy,soilenerg&
&y=grid%soilenergy,snowenergy=grid%snowenergy,canhsxy=grid%canhsxy,ACC_DWATERXY=grid%ACC_DWATERXY,ACC_PRCPXY=grid%ACC_PRCPXY,ACC_EC&
&ANXY=grid%ACC_ECANXY,ACC_ETRANXY=grid%ACC_ETRANXY,ACC_EDIRXY=grid%ACC_EDIRXY,IRFRACT=grid%IRFRACT,SIFRACT=grid%SIFRACT,MIFRACT=gri&
&d%MIFRACT,FIFRACT=grid%FIFRACT,IRNUMSI=grid%IRNUMSI,IRNUMMI=grid%IRNUMMI,IRNUMFI=grid%IRNUMFI,IRWATSI=grid%IRWATSI,IRWATMI=grid%IR&
&WATMI,IRWATFI=grid%IRWATFI,IRELOSS=grid%IRELOSS,IRSIVOL=grid%IRSIVOL,IRMIVOL=grid%IRMIVOL,IRFIVOL=grid%IRFIVOL,IRRSPLH=grid%IRRSPL&
&H,QTDRAIN=grid%QTDRAIN,TD_FRACTION=grid%TD_FRACTION,smcwtdxy=grid%smcwtdxy,rechxy=grid%rechxy,deeprechxy=grid%deeprechxy,fdepthxy=&
&grid%fdepthxy,areaxy=grid%areaxy,rivercondxy=grid%rivercondxy,riverbedxy=grid%riverbedxy,eqzwt=grid%eqzwt,pexpxy=grid%pexpxy,qrfxy&
&=grid%qrfxy,qspringxy=grid%qspringxy,qslatxy=grid%qslatxy,qrfsxy=grid%qrfsxy,qlatxy=grid%qlatxy,qspringsxy=grid%qspringsxy,smoiseq&
&=grid%smoiseq,wtddt=config_flags%wtddt,stepwtd=grid%stepwtd,gecros_state=grid%gecros_state,ua_phys=config_flags%ua_phys,flx4=grid%&
&flx4,fvb=grid%fvb,fbur=grid%fbur,fgsn=grid%fgsn,IDS=ids,IDE=ide,JDS=jds,JDE=jde,KDS=kds,KDE=kde,IMS=ims,IME=ime,JMS=jms,JME=jme,KM&
&S=kms,KME=kme,IPS=ips,IPE=ipe,JPS=jps,JPE=jpe,KPS=kps,KPE=kpe,I_START=grid%i_start,I_END=min(grid%i_end,ide-1),J_START=grid%j_star&
&t,J_END=min(grid%j_end,jde-1),KTS=k_start,KTE=min(k_end,kde-1),NUM_TILES=grid%num_tiles,te_temf=grid%te_temf,hd_temf=grid%hd_temf,&
&fCor=grid%f,exch_temf=exch_temf,wm_temf=grid%wm_temf,hfx_force=grid%hfx_force,lh_force=grid%lh_force,tsk_force=grid%tsk_force,hfx_&
&force_tend=grid%hfx_force_tend,lh_force_tend=grid%lh_force_tend,tsk_force_tend=grid%tsk_force_tend,QV_CURR=moist(ims,kms,jms,P_QV)&
&,F_QV=F_QV,QC_CURR=moist(ims,kms,jms,P_QC),F_QC=F_QC,QR_CURR=moist(ims,kms,jms,P_QR),F_QR=F_QR,QI_CURR=moist(ims,kms,jms,P_QI),F_Q&
&I=F_QI,QS_CURR=moist(ims,kms,jms,P_QS),F_QS=F_QS,QG_CURR=moist(ims,kms,jms,P_QG),F_QG=F_QG,CAPG=grid%capg,EMISS=grid%emiss,HOL=hol&
&,MOL=grid%mol,T2OBS=grid%t2obs,Q2OBS=grid%q2obs,RAINBL=grid%rainbl,SR=grid%sr,RAINSHV=grid%rainshv,GRAUPELNCV=grid%graupelncv,HAIL&
&NCV=grid%hailncv,RAINNCV=grid%rainncv,REGIME=grid%regime,T2=grid%t2,THC=grid%thc,QSG=grid%qsg,QVG=grid%qvg,QCG=grid%qcg,SOILT1=gri&
&d%soilt1,TSNAV=grid%tsnav,SMFR3D=grid%smfr3d,KEEPFR3DFLAG=grid%keepfr3dflag,DEW=grid%dew,POTEVP=grid%POTEVP,SNOPCX=grid%SNOPCX,SOI&
&LTB=grid%SOILTB,rhosnf=grid%rhosnf,precipfr=grid%precipfr,snowfallac=grid%snowfallac,MOSAIC_LU=config_flags%mosaic_lu,MOSAIC_SOIL=&
&config_flags%mosaic_soil,ISURBAN=isurban,MMINLU=TRIM(mminlu),SNOTIME=grid%SNOTIME,RDLAI2D=config_flags%rdlai2d,usemonalb=config_fl&
&ags%usemonalb,NOAHRES=grid%noahres,TSK_SAVE=grid%tsk_save,ch=grid%ch,fgdp=grid%fgdp,dfgdp=grid%dfgdp,vdfg=grid%vdfg,grav_settling=&
&config_flags%grav_settling,OM_TMP=grid%om_tmp,OM_S=grid%om_s,OM_U=grid%om_u,OM_V=grid%om_v,OM_DEPTH=grid%om_depth,OM_ML=grid%OM_ML&
&,OM_LON=grid%om_lon,OM_LAT=grid%om_lat,okms=1,okme=config_flags%ocean_levels,rdx=grid%rdx,rdy=grid%rdy,msfu=grid%msfu,msfv=grid%ms&
&fv,msft=grid%msft,XTIME=grid%xtime,OM_TINI=grid%om_tini,OM_SINI=grid%om_sini,id=grid%id,omdt=config_flags%omdt,sf_surface_mosaic=c&
&onfig_flags%sf_surface_mosaic,mosaic_cat=config_flags%mosaic_cat,mosaic_cat_index=grid%mosaic_cat_index,landusef2=grid%landusef2,T&
&SK_mosaic=grid%TSK_mosaic,QSFC_mosaic=grid%QSFC_mosaic,TSLB_mosaic=grid%TSLB_mosaic,SMOIS_mosaic=grid%SMOIS_mosaic,SH2O_mosaic=gri&
&d%SH2O_mosaic,CANWAT_mosaic=grid%CANWAT_mosaic,SNOW_mosaic=grid%SNOW_mosaic,SNOWH_mosaic=grid%SNOWH_mosaic,SNOWC_mosaic=grid%SNOWC&
&_mosaic,ALBEDO_mosaic=grid%ALBEDO_mosaic,ALBBCK_mosaic=grid%ALBBCK_mosaic,EMISS_mosaic=grid%EMISS_mosaic,EMBCK_mosaic=grid%EMBCK_m&
&osaic,ZNT_mosaic=grid%ZNT_mosaic,Z0_mosaic=grid%Z0_mosaic,HFX_mosaic=grid%HFX_mosaic,QFX_mosaic=grid%QFX_mosaic,LH_mosaic=grid%LH_&
&mosaic,GRDFLX_mosaic=grid%GRDFLX_mosaic,SNOTIME_mosaic=grid%SNOTIME_mosaic,RS_mosaic=grid%RS_mosaic,LAI_mosaic=grid%LAI_mosaic,TR_&
&URB2D_mosaic=grid%TR_URB2D_mosaic,TB_URB2D_mosaic=grid%TB_URB2D_mosaic,TG_URB2D_mosaic=grid%TG_URB2D_mosaic,TC_URB2D_mosaic=grid%T&
&C_URB2D_mosaic,QC_URB2D_mosaic=grid%QC_URB2D_mosaic,UC_URB2D_mosaic=grid%UC_URB2D_mosaic,TRL_URB3D_mosaic=grid%TRL_URB3D_mosaic,TB&
&L_URB3D_mosaic=grid%TBL_URB3D_mosaic,TGL_URB3D_mosaic=grid%TGL_URB3D_mosaic,SH_URB2D_mosaic=grid%SH_URB2D_mosaic,LH_URB2D_mosaic=g&
&rid%LH_URB2D_mosaic,G_URB2D_mosaic=grid%G_URB2D_mosaic,RN_URB2D_mosaic=grid%RN_URB2D_mosaic,TS_URB2D_mosaic=grid%TS_URB2D_mosaic,T&
&S_RUL2D_mosaic=grid%TS_RUL2D_mosaic,ZOL=grid%ZOL,SDA_HFX=grid%SDA_HFX,SDA_QFX=grid%SDA_QFX,HFX_BOTH=grid%HFX_BOTH,QFX_BOTH=grid%QF&
&X_BOTH,QNORM=grid%QNORM,fasdas=config_flags%fasdas,XLAIDYN=grid%XLAIDYN,spp_lsm=config_flags%spp_lsm,pattern_spp_lsm=grid%pattern_&
&spp_lsm,field_sf=grid%field_sf,spp_pbl=config_flags%spp_pbl,pattern_spp_pbl=grid%pattern_spp_pbl,multi_perturb=config_flags%multi_&
&perturb,pert_noah=config_flags%pert_noah,perts_qvapor=grid%pert3d(:,:,:,P_PQVAPOR),perts_th=grid%pert3d(:,:,:,P_PTH),perts_smois=g&
&rid%pert3d(:,:,:,P_PSMOIS),perts_tsoil=grid%pert3d(:,:,:,P_PTSOIL),pert_noah_qv=config_flags%pert_noah_qv,pert_noah_t=config_flags&
&%pert_noah_t,pert_noah_smois=config_flags%pert_noah_smois,pert_noah_tslb=config_flags%pert_noah_tslb)
...
&ztop_plume=grid%ztop_plume,ktop_plume=grid%ktop_plume,spp_pbl=config_flags%spp_pbl,pattern_spp_pbl=grid%pattern_spp_pbl,restart=co&
&nfig_flags%restart,cycling=config_flags%cycling,pek=grid%pek_pbl,pep=grid%pep_pbl,PEK_ADV=scalar(ims,kms,jms,P_pek_adv),PEP_ADV=sc&
&alar(ims,kms,jms,P_pep_adv),GWD_OPT=config_flags%gwd_opt,gwd_diags=config_flags%gwd_diags,DTAUX3D=grid%dtaux3d,DTAUY3D=grid%dtauy3&
&d,DUSFCG=grid%dusfcg,DVSFCG=grid%dvsfcg,VAR2D=grid%var2d,OC12D=grid%oc12d,OA1=grid%oa1,OA2=grid%oa2,OA3=grid%oa3,OA4=grid%oa4,OL1=&
&grid%ol1,OL2=grid%ol2,OL3=grid%ol3,OL4=grid%ol4,SINA=grid%sina,COSA=grid%cosa,dtaux3d_ls=grid%dtaux3d_ls,dtauy3d_ls=grid%dtauy3d_l&
&s,dtaux3d_bl=grid%dtaux3d_bl,dtauy3d_bl=grid%dtauy3d_bl,dtaux3d_ss=grid%dtaux3d_ss,dtauy3d_ss=grid%dtauy3d_ss,dtaux3d_fd=grid%dtau&
&x3d_fd,dtauy3d_fd=grid%dtauy3d_fd,DUSFCG_ls=grid%dusfcg_ls,DVSFCG_ls=grid%dvsfcg_ls,DUSFCG_bl=grid%dusfcg_bl,DVSFCG_bl=grid%dvsfcg&
&_bl,DUSFCG_ss=grid%dusfcg_ss,DVSFCG_ss=grid%dvsfcg_ss,DUSFCG_fd=grid%dusfcg_fd,DVSFCG_fd=grid%dvsfcg_fd,VAR2DLS=grid%var2dls,OC12D&
&LS=grid%oc12dls,OA1LS=grid%oa1ls,OA2LS=grid%oa2ls,OA3LS=grid%oa3ls,OA4LS=grid%oa4ls,OL1LS=grid%ol1ls,OL2LS=grid%ol2ls,OL3LS=grid%o&
&l3ls,OL4LS=grid%ol4ls,VAR2DSS=grid%var2dss,OC12DSS=grid%oc12dss,OA1SS=grid%oa1ss,OA2SS=grid%oa2ss,OA3SS=grid%oa3ss,OA4SS=grid%oa4s&
&s,OL1SS=grid%ol1ss,OL2SS=grid%ol2ss,OL3SS=grid%ol3ss,OL4SS=grid%ol4ss,MFSHCONV=config_flags%mfshconv,MASSFLUX_EDKF=grid%massflux_E&
...
&g_u,multi_perturb=config_flags%multi_perturb,pert_mynn=config_flags%pert_mynn,perts_qvapor=grid%pert3d(:,:,:,P_PQVAPOR),perts_qclo&
&ud=grid%pert3d(:,:,:,P_PQCLOUD),perts_th=grid%pert3d(:,:,:,P_PTH),perts_tke=grid%pert3d(:,:,:,P_PTKE),pert_mynn_qv=config_flags%pe&
&rt_mynn_qv,pert_mynn_qc=config_flags%pert_mynn_qc,pert_mynn_t=config_flags%pert_mynn_t,pert_mynn_qke=config_flags%pert_mynn_qke,Z_&
&AT_W=grid%z_at_w,CLDFRA_OLD_MP=grid%cldfra_old_mp,CLDFRA=grid%cldfra,RTHRATENLW=grid%rthratenlw,TAURESX2D=grid%tauresx2d,TAURESY2D&
&=grid%tauresy2d,TPERT2D=grid%tpert2d,QPERT2D=grid%qpert2d,WPERT2D=grid%wpert2d,WSEDL3D=grid%wsedl3d,TURBTYPE3D=grid%turbtype3d,SMA&
&W3D=grid%smaw3d,QNC_CURR=scalar(ims,kms,jms,P_QNC),F_QNC=f_qnc,QNI_CURR=scalar(ims,kms,jms,P_QNI),F_QNI=f_qni,RQNIBLTEN=grid%rqnib&
&lten,XLAT_V=grid%xlat_v,XLONG_V=grid%xlong_v,FNM=grid%fnm,FNP=grid%fnp,IS_CAMMGMP_USED=grid%is_CAMMGMP_used,WSTAR=grid%wstar_ysu,D&
&ELTA=grid%delta_ysu,SCALAR=scalar,SCALAR_TEND=scalar_tend,NUM_SCALAR=num_scalar,TRACER=tracer,TRACER_TEND=tracer_tend,NUM_TRACER=n&
&um_tracer,SCALAR_PBLMIX=config_flags%scalar_pblmix,TRACER_PBLMIX=config_flags%tracer_pblmix,QNORM=grid%QNORM,fasdas=config_flags%f&
&asdas)
