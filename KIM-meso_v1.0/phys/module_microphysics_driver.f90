...
SUBROUTINE microphysics_driver(                                          &
   ...
   USE module_state_description, ONLY :                                  &
                    ...
                    ,GSFCGCESCHEME, WDM5SCHEME, WDM6SCHEME, KDM6SCHEME, KDM6ADSCHEME, NSSL_2MOM, MADWRF_MP  &  
                    ,FER_MP_HIRES_ADVECT & 
  ...
  USE module_dm, ONLY : &
   ...
   USE module_mp_wdm6
   USE module_mp_kdm6
   USE module_mp_kdm6ad, ONLY: kdm6ad
   USE module_mp_wdm7
...
REAL, DIMENSION( ims:ime, kms:kme, jms:jme ),                        &
   ...
   REAL, INTENT(IN) :: scale_h, ncmin_land, ncmin_sea                       
   REAL, OPTIONAL, DIMENSION( ims:ime , jms:jme ) , INTENT(OUT) :: refl_sfc 
...
'arguments not present for calling wdm6')
        ...
        CASE (KDM6SCHEME)
             CALL wrf_debug ( 100 , 'microphysics_driver: calling kdm6' )
             IF ( PRESENT( QV_CURR ) .AND. PRESENT ( QC_CURR ) .AND.  &
                  PRESENT( QR_CURR ) .AND. PRESENT ( QI_CURR ) .AND.  &
                  PRESENT( QS_CURR ) .AND. PRESENT ( QG_CURR ) .AND.  &
                  PRESENT( QNN_CURR ) .AND. PRESENT ( QNC_CURR ) .AND.&
                  PRESENT( QNI_CURR ).AND.  PRESENT( QNR_CURR ).AND.  &
                  PRESENT( QIB_CURR).AND.PRESENT( RHOPO3D).AND.       &
                  PRESENT( RAINNC  ) .AND. PRESENT ( RAINNCV )  ) THEN
             CALL kdm6(                                             &
                  TH=th                                             &    
                 ,Q=qv_curr                                         &    
...
'arguments not present for calling kdm6')
             ENDIF

        CASE (KDM6ADSCHEME)
             WRITE(0,*) 'KDM6AD_PHASE driver case enter itimestep=', itimestep
             FLUSH(0)
             CALL wrf_debug ( 100 , 'microphysics_driver: calling kdm6ad' )
             IF ( PRESENT( QV_CURR ) .AND. PRESENT ( QC_CURR ) .AND.  &
                  PRESENT( QR_CURR ) .AND. PRESENT ( QI_CURR ) .AND.  &
                  PRESENT( QS_CURR ) .AND. PRESENT ( QG_CURR ) .AND.  &
                  PRESENT( QNN_CURR ) .AND. PRESENT ( QNC_CURR ) .AND.&
                  PRESENT( QNI_CURR ).AND.  PRESENT( QNR_CURR ).AND.  &
                  PRESENT( QIB_CURR) ) THEN
             WRITE(0,*) 'KDM6AD_PHASE driver present-ok before call'
             FLUSH(0)
             CALL kdm6ad(                                           &
                  TH=th                                             &
                 ,Q=qv_curr                                         &
             ...
             WRITE(0,*) 'KDM6AD_PHASE driver returned from kdm6ad'
             FLUSH(0)
             ELSE
                 CALL wrf_error_fatal3("<stdin>",2448,&
'arguments not present for calling kdm6ad')
             ENDIF
...
wrf_err_message )
   ...
   DO ij = 1 , num_tiles
     ...
     DO j = jts, jte
     DO i = its, ite
       refl_sfc(i,j) = refl_10cm(i,kts,j)
       ENDDO
       ENDDO
