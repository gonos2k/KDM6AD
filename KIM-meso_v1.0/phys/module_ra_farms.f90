module module_ra_farms
    ...
    real, parameter :: COSZEN_MIN = 0.001         
    real, parameter :: ALBEDO_MAX = 0.95          
    real, parameter :: DENOM_MIN = 0.05           
  ...
  contains
    ...
    subroutine Farms_driver (ims, ime, jms, jme, its, ite, jts, jte, kms, kme, kts, kte,  &
      ...
      real    :: coszen_safe, ramp_factor, albedo_safe
      ...
      j_loop: do j = jts, jte
         i_loop: do i = its, ite
           daytime_if: if (coszen_loc(i, j) <= 0.0 ) then
             ...
             coszen_safe = max(coszen_loc(i, j), COSZEN_MIN)
             ...
             albedo_safe = min(albedo(i, j), ALBEDO_MAX)
             
             

             Call Farms (p8w(i, 1, j), albedo_safe, aerssa2d(i, j),         &
                 aerasy2d(i, j), coszen_safe, beta,                          &
             ...
             if (coszen_loc(i, j) < 0.1) then  
               ramp_factor = coszen_loc(i, j) / 0.1
               swdown2(i, j) = swdown2(i, j) * ramp_factor
               swddni2(i, j) = swddni2(i, j) * ramp_factor
               swddir2(i, j) = swddir2(i, j) * ramp_factor
               swddif2(i, j) = swddif2(i, j) * ramp_factor
               swdownc2(i, j) = swdownc2(i, j) * ramp_factor
               swddnic2(i, j) = swddnic2(i, j) * ramp_factor
             end if
             
             
             
             
             swdown2(i, j) = min(swdown2(i, j), 1361.0 * coszen_loc(i, j) * 1.5)
             swddni2(i, j) = min(swddni2(i, j), 1361.0 * coszen_loc(i, j))
    ...
    subroutine FARMS (p_pa, albdo, ssa, g, solarangle, beta, alpha, w_mm, &
      ...
      real :: de_cloud, de_cloud2, de_ice, de_ice2, de_snow, de_snow2, f0, f1, ftotal, ozone, &
          ...
          w, Z, tau_tot
      real :: solarangle_safe


      PI = acos(-1.0)
      
      
      solarangle_safe = max(solarangle, COSZEN_MIN)
      ...
      Z = acos(solarangle_safe) * 180.0 / PI
      ...
      if (tau_qv > TAU_MIN) then
        Call Watermodel (tau_tot, de_cloud2, solarangle_safe, Tducld_water, Ruucld_water)
      else
        Tducld_water = 0.0
        Ruucld_water = 0.0
        tau_qv2 = 0.0
      ...
      if (tau_qi > TAU_MIN) then
        call Icemodel (tau_tot, de_ice2, solarangle_safe, Tducld_ice, Ruucld_ice)
      else
        Tducld_ice = 0.0
        Ruucld_ice = 0.0
        tau_qi2 = 0.0
      ...
      if (tau_qs > TAU_MIN) then
        call Icemodel (tau_tot, de_snow2, solarangle_safe, Tducld_snow, Ruucld_snow)
      else
        Tducld_snow = 0.0
        Ruucld_snow = 0.0
        tau_qs2 = 0.0
      ...
      Tddcld = exp(-tau/solarangle_safe)
      ...
      Ftotal = F1/max(1.0-albdo*(Ruuclr + Ruucld*Tuuclr*Tuuclr), DENOM_MIN)
      ghi = Ftotal
      ghi_clear =  solarangle * F0 * ((Tddclr + Tduclr) / max(1.0 - albdo * Ruuclr, DENOM_MIN))
    ...
    subroutine BIRD(p, albdo, SSA, g, Z, Radius, beta,&
      ...
      Ftotal = ( Fddclr + Fdif )/max(1.0 - albdo*Ruuclr, DENOM_MIN)
    ...
    subroutine WATERMODEL( tau, De, solarangle, Tducld, Ruucld )
      ...
      Ptau = (2.8850+0.002*(De-60.0))*solarangle_safe-0.007347
      Ptau = max(Ptau, PTAU_MIN)
      PDHI = (1.0+(De-60.0)*0.0002)*1087.24*solarangle_safe**1.1605

      delta = -0.644531*solarangle_safe+1.20117+0.129807/solarangle_safe &
          -0.00121096/(solarangle_safe*solarangle_safe) + &
          1.52587e-07/(solarangle_safe*solarangle_safe*solarangle_safe)
      delta = Max (delta, DELTA_MIN)

      y = 0.012*(tau-Ptau)*solarangle_safe
      PPDHI = (1.0+SINH(y))*PDHI*&
          exp(-( (log10(tau)-log10(Ptau))**2.0 )/delta)

      Tducld = PPDHI/(solarconst*solarangle_safe)
    ...
    subroutine ICEMODEL( tau, De, solarangle, Tducld, Ruucld )
      ...
      if ( De .le. 26.0 ) then 
        Ptau = 2.8487*solarangle_safe- 0.0029
      endif
      if ( De .gt. 26.0 ) then 
        Ptau =  (2.8355 + (100.0-De)*0.006)*solarangle_safe - 0.00612
      endif
      Ptau = max(Ptau, PTAU_MIN)

      PDHI = 1047.6367*solarangle_safe**1.0883

      delta = -0.0549531*solarangle_safe+0.617632+0.17876/(solarangle_safe) &
          -0.002174/(solarangle_safe*solarangle_safe) 
      delta = Max (delta, DELTA_MIN)

      y = 0.01*(tau-Ptau)*solarangle_safe
      PPDHI = (1.0+SINH(y))*PDHI*&
          exp(-( (log10(tau)-log10(Ptau))**2.0 )/delta)
