     ...
     &             ((temg(mgs) - cbw)**2))
         ...
         ssmx = ssmxinit
          ...
          IF ( ssf(mgs) > ssmx  .and. ssf(mgs) < 20.0 .and.  &
             ( ccnc(mgs) > 0.05*cwnccn(mgs) .or. ( ac_opt > 0 .and. ccnc_ac(mgs) - cx(mgs,lc) > 0.0 ) ) ) THEN 
 ...
 2200 continue
   ...
   END SUBROUTINE NUCOND
     ...
     & )
      ...
      real ac1,bc, taus, c1,d1,e1,f1,p380,tmp,tmp1,tmp2,tmp3,tmp4,tmp5,tmp6,temp3 
      real :: flim
      real dw,dwr
       ...
       DO mgs = 1,ngscnt
          DO il = lh,lhab 
            ...
            IF ( qx(mgs,il) > qxmin(il) .and. ivshdgs > 0 ) THEN
              ...
              tmpdiam = (shedalp+alpha(mgs,il))*xdia(mgs,il,1) 
