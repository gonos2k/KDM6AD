...
CONTAINS
   ...
   USE module_cu_ksas2  , ONLY : cu_ksas2 
     ...
     CASE (KSASSCHEME)
        IF ( PRESENT ( QFX ) .AND. PRESENT( HFX ) ) THEN
          ...
          CALL CU_KSAS(                                         &
               ...
               ,IMS=ims,IME=ime,JMS=jms,JME=jme,KMS=kms,KME=kme &
               ,ITS=its,ITE=ite,JTS=jts,JTE=jte,KTS=kts,KTE=kte &
               ,SIGMA=grid%sigma,THRE=grid%thre                           & 
                                                                )
        ELSE
...
'Lacking arguments for CU_KSAS in cumulus driver')
      ...
      CASE (KSAS2SCHEME)
         IF ( PRESENT ( QFX ) .AND. PRESENT( HFX ) ) THEN
           CALL wrf_debug(100,'in ksas2_cps')
           CALL CU_KSAS2(                                        &
                 DT=dt,DX=dx,P3DI=p8w,P3D=p,PI3D=pi,             &
                 QC3D=QC_CURR,QI3D=QI_CURR,RHO3D=rho,            &
                 ITIMESTEP=itimestep,STEPCU=STEPCU,              &
...
'Lacking arguments for CU_KSAS2 in cumulus driver')
         ENDIF
