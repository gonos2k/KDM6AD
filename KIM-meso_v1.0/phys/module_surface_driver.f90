...
CONTAINS
   ...
   REAL, DIMENSION( ims:ime, jms:jme ) , INTENT(INOUT)::   chs_jun
   REAL, DIMENSION( ims:ime, jms:jme ) , INTENT(INOUT)::   zol_Jun
   REAL, DIMENSION( ims:ime, jms:jme ) , INTENT(INOUT)::   br_Jun
   ...
   REAL, DIMENSION( ims:ime, jms:jme ), INTENT(OUT)::   U80 
   REAL, DIMENSION( ims:ime, jms:jme ), INTENT(OUT)::   U140
   REAL, DIMENSION( ims:ime, jms:jme ), INTENT(OUT)::   U220
   REAL, DIMENSION( ims:ime, jms:jme ), INTENT(OUT)::   V10
   REAL, DIMENSION( ims:ime, jms:jme ), INTENT(OUT)::   V80 
   REAL, DIMENSION( ims:ime, jms:jme ), INTENT(OUT)::   V140
   REAL, DIMENSION( ims:ime, jms:jme ), INTENT(OUT)::   V220
  ...
  REAL, DIMENSION( ims:ime, 1:num_soil_layers*mosaic_cat, jms:jme ), OPTIONAL, INTENT(INOUT)::   &
   ...
   REAL, OPTIONAL, DIMENSION( ims:ime, 1:num_soil_layers*mosaic_cat, jms:jme ), INTENT(INOUT) :: TGL_URB3D_mosaic
    ...
    real,    dimension(ims:ime,jms:jme ),intent(inout) :: water_depth, charnock 
...
"Field ALBSI not found in input.  Field ALBSI is required if SEAICE_ALBEDO_OPT=2")
  ...
  isisfc = ( FRACTIONAL_SEAICE .EQ. 1  .AND. (          &
            ...
            (sf_sfclay_physics .EQ. YSLSCHEME ) .OR. &  
       ...
       IF (PRESENT(qv_curr)                            .AND.    &    
         ...
         IF ( FRACTIONAL_SEAICE == 1 ) THEN 
             CALL SFCLAYREV_SEAICE_WRAPPER(u_phytmp,v_phytmp,t_phy,qv_curr,&
                  ...
                  shalwater_z0,water_depth,charnock,                  &    
                  scm_force_flux,sf_surface_physics,errmsg,errflg   ) 
              CALL wrf_debug( 100, 'ysl1' )
       ELSE 
              CALL wrf_debug( 100, 'in call YSL' )
              call wrf_debug( 100, 'CHARNOCK UPDATE =', charnock_update)
              if(charnock_update == 0) charnock = 0.0185                   
              CALL YSL(u_phytmp,v_phytmp,t_phy,qv_curr,                  &    
                   p_phy,dz8w,cp,g,rcp,r_d,xlv,psfc,chs,chs2,cqs2,cpm,   &
                   znt,ust,pblh,mavail,zol,mol,regime,psim,psih,fm,fhh,  &
                   xland,hfx,qfx,lh,tsk,flhc,flqc,qgh,qsfc,rmol,         &    
                   u10,v10,th2,t2,q2,                                    &    
          ...
          DO j = j_start(ij),j_end(ij)
           ...
           ENDDO
          ENDDO
          CALL wrf_debug( 100, 'end2 YSL' )

          ENDIF
        ELSE
          CALL wrf_error_fatal3("<stdin>",2152,&
'Lacking arguments for YSL in surface driver')
        ENDIF
...
'Lacking arguments for P-X LSM in surface driver')
     ...
     IF (MYJ) call wrf_error_fatal3("<stdin>",3726,&
   ...
   IF (sf_ocean_physics .EQ. OMLSCHEME .or. sf_ocean_physics .EQ. PWP3DSCHEME) THEN
     ...
     DO ij = 1 , num_tiles
        CALL ocean_driver(tml,t0ml,hml,h0ml,huml,hvml,ust,u_phy,v_phy, &
                     tmoml,f,g,oml_gamma,                         &
                     xland,lakemask,hfx,lh,tsk,gsw,glw,emiss,     & 
