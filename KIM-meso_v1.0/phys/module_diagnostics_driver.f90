...
CONTAINS
   ...
   SUBROUTINE diagnostics_driver ( grid, config_flags,                  &
      ...
      USE module_diag_fogvis, ONLY : diag_fogvis
...
diag_message )
      ...
      FOGVIS_DIAGS: IF ( config_flags%fogvis_opt /= 0 ) THEN
         !$OMP PARALLEL DO   &
         !$OMP PRIVATE ( ij )
         DO ij = 1 , grid%num_tiles
            CALL diag_fogvis(                                         &
                 ...
                 grid%u10, grid%v10, grid%ust,                         &
                 grid%hfx, grid%qfx, grid%pblh,                        &
                 grid%p, grid%pb, grid%t_2,                            &
                 grid%ph_2, grid%phb,                                  &
                 moist, num_moist,                                     &
                 scalar, num_scalar,                                   &
                 grid%beta_aer_sfc, grid%aod_sfc,                      &
                 grid%vis_sfc, grid%vis_sfc_raw, grid%vis_sfc_capped,   &
                 grid%fogfrac_sfc,                                    &
                 grid%fogmask, grid%fogbase_m, grid%fogtop_m, grid%fogdepth_m, &
                 grid%betaext_sfc, grid%lwc_sfc_gm3, grid%nd_sfc_cm3, grid%re_sfc_um )
         END DO
         !$OMP END PARALLEL DO
      ENDIF FOGVIS_DIAGS
