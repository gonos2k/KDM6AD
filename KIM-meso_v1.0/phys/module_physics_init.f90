...
CONTAINS
   ...
   REAL,    DIMENSION( ims:ime, jms:jme )                     , &
            OPTIONAL, INTENT(IN)    ::                     U10, &
                                                           V10, u80, v80, u140, v140, u220, v220 
   REAL,    DIMENSION( ims:ime, jms:jme )                     , &
  ...
  REAL,     DIMENSION( ims:ime , kms:kme , jms:jme ) , INTENT(OUT), OPTIONAL :: &
   ...
   REAL,     DIMENSION( ims:ime , jms:jme ) , INTENT(INOUT) ::  tree_hgt, hc_jun 
   ...
   REAL, INTENT(INOUT) :: scale_h 
   REAL, INTENT(INOUT) :: ncmin_land, ncmin_sea 
  ...
  REAL,              DIMENSION( ims:ime, jms:jme ), INTENT(INOUT) ::  charnock 
  ...
  real, OPTIONAL,  dimension(ims:ime,1:maxpatch,jms:jme ),intent(inout) ::  &
   ...
   REAL    :: treecoef(12,21), tree_alpha
   INTEGER :: imonth, itree_canopy
...
CHARACTER(LEN=8) :: name
   ...
   if (( (config_flags%ra_lw_physics .eq. RRTMG_LWSCHEME ) .or. &
       ...
       ( (config_flags%ra_sw_physics .eq. RRTMG_SWSCHEME ) .or. &
        ...
        (config_flags%mp_physics  .eq. THOMPSON .or.        &
         ...
         config_flags%mp_physics  .eq. KDM6SCHEME .or.      & 
         config_flags%mp_physics  .eq. KDM6ADSCHEME .or.    &
   ...
   IF ( config_flags%swint_opt .eq. 2 ) THEN
      IF ((   config_flags%mp_physics == THOMPSON .OR.      &
              ...
              config_flags%mp_physics == KDM6SCHEME .OR.    &  
              config_flags%mp_physics == KDM6ADSCHEME ).OR. &
...
'--- ERROR: FARMS (swint_opt==2) requires a different MP scheme (Please see the module_physics_init')
   ...
   WRITE(0,*) 'KDM6AD_PHASE phy_init top mp_physics=', config_flags%mp_physics
   FLUSH(0)
   WRITE(0,*) 'KDM6AD_PHASE phy_init before start_of_simulation write'
   FLUSH(0)

   WRITE(wrf_err_message,*) 'phy_init:  start_of_simulation = ',start_of_simulation
   WRITE(0,*) 'KDM6AD_PHASE phy_init after start_of_simulation write'
   FLUSH(0)
   CALL wrf_debug (100, TRIM(wrf_err_message))
   WRITE(0,*) 'KDM6AD_PHASE phy_init after start_of_simulation debug'
   FLUSH(0)
   ...
   WRITE(0,*) 'KDM6AD_PHASE phy_init before nl_get_cen_lat'
   FLUSH(0)
   CALL nl_get_cen_lat(id,cen_lat)
   WRITE(0,*) 'KDM6AD_PHASE phy_init after nl_get_cen_lat'
   FLUSH(0)
   CALL wrf_debug(100,'calling nl_get_iswater, nl_get_isice, nl_get_mminlu_loc')
   WRITE(0,*) 'KDM6AD_PHASE phy_init before nl_get_iswater'
   FLUSH(0)
   CALL nl_get_iswater(id,iswater)
   WRITE(0,*) 'KDM6AD_PHASE phy_init after nl_get_iswater'
   FLUSH(0)
   CALL nl_get_isice(id,isice)
   WRITE(0,*) 'KDM6AD_PHASE phy_init after nl_get_isice'
   FLUSH(0)
   CALL nl_get_isurban(id,isurban)
   WRITE(0,*) 'KDM6AD_PHASE phy_init after nl_get_isurban'
   FLUSH(0)

   CALL nl_get_mminlu( id, mminlu_loc )
   WRITE(0,*) 'KDM6AD_PHASE phy_init after nl_get_mminlu'
   FLUSH(0)

   CALL wrf_debug(100,'after nl_get_iswater, nl_get_isice, nl_get_mminlu_loc')
   WRITE(0,*) 'KDM6AD_PHASE phy_init after nl_get block before landuse_ISICE'
   FLUSH(0)

   landuse_ISICE = isice
   WRITE(0,*) 'KDM6AD_PHASE phy_init after landuse_ISICE'
   FLUSH(0)
 ...
 ENDIF
  ...
  enddo
   ...
   WRITE(0,*) 'KDM6AD_PHASE phy_init before landuse_init block'
   FLUSH(0)
   ...
   IF(mminlu_loc .ne. '    ')THEN
     ...
     CALL landuse_init(lu_index, snowc, albedo, albbck, snoalb, mavail, emiss, embck, &
                ...
                ims, ime, jms, jme, kms, kme,                       &
                its, ite, jts, jte, kts, kte                       )
     WRITE(0,*) 'KDM6AD_PHASE phy_init after landuse_init call'
     FLUSH(0)
   ENDIF
  ...
  ENDIF 
   ...
   CALL wrf_debug(100,'TREE CANOPY = start')
   WRITE(0,*) 'KDM6AD_PHASE phy_init tree canopy start'
   FLUSH(0)

   CALL nl_get_tree_canopy(id, itree_canopy)
   CALL nl_get_tree_canopy_alpha(id, tree_alpha)
   IF (itree_canopy .ne. 0) then
      if(itree_canopy .eq. 1) treecoef(:,:) = tree_alpha
      if(itree_canopy .eq. 2) call tree_canopy_IGBP_init(treecoef)
      CALL nl_get_start_month(id,imonth)
      DO j=jts,jtf
         DO i=its,itf
           if ((xland(i,j) .lt. 1.5) .and. (tree_hgt(i,j) .gt. 0.)) then
               HC_jun(i,j)=tree_hgt(i,j)
               ZNT(i,j)=HC_jun(i,j)*treecoef(imonth,lu_index(i,j))
               Z0(i,j) =ZNT(i,j)
           endif
     ...
     CALL wrf_debug(100,'TREE CANOPY = end')
     WRITE(0,*) 'KDM6AD_PHASE phy_init tree canopy end'
     FLUSH(0)
   ...
   WRITE(0,*) 'KDM6AD_PHASE phy_init before z2sigma'
   FLUSH(0)
   CALL z2sigma(zfull,zhalf,sfull,shalf,p_top,pptop,config_flags, &
                allowed_to_read,                                  &
                kds,kde,kms,kme,kts,kte)
   WRITE(0,*) 'KDM6AD_PHASE phy_init after z2sigma'
   FLUSH(0)
  ...
  ENDIF
   ...
   WRITE(0,*) 'KDM6AD_PHASE phy_init before ra_init'
   FLUSH(0)
...
TRIM ( wrf_err_message ) )
   ...
   IF ( config_flags%o3input .EQ. 2 .AND. id .EQ. 1 ) THEN
      ...
      CALL wrf_debug(100,'OZNINI = start')
      CALL oznini(ozmixm,pin,levsiz,n_ozmixm,XLAT,                &
                     ids, ide, jds, jde, kds, kde,                  &
                     ims, ime, jms, jme, kms, kme,                  &
                     its, ite, jts, jte, kts, kte)
      CALL wrf_debug(100,'OZNINI = end')
   ENDIF
...
'arguments not present for calling CLM' )
   ...
   pbl_select: SELECT CASE(config_flags%bl_pbl_physics)
      ...
      CASE (SHINHONGSCHEME)
           if(isfc .ne. 1)CALL wrf_error_fatal &
            ( 'module_physics_init: Use sf_sfclay_physics= 1 or 91 or 37 for this pbl option' )
   ...
   cps_select: SELECT CASE(config_flags%cu_physics)
     ...
     CASE (GDSCHEME)
          CALL gdinit(RTHCUTEN,RQVCUTEN,RQCCUTEN,RQICUTEN,        &
                      ...
                      ims, ime, jms, jme, kms, kme,               &
                      its, ite, jts, jte, kts, kte                )
     CASE (KSASSCHEME,NSASSCHEME,KSAS2SCHEME)                        
...
'arguments not present for calling camzmscheme' )
  ...
  INTEGER :: i,j,itf,jtf
   ...
   mp_select: SELECT CASE(config_flags%mp_physics)
     ...
     CASE (KDM6SCHEME)
          CALL kdm6init(rhoair0,rhowater,rhosnow,cliq,cpv,ccn_conc, &
                config_flags%hail_opt, allowed_to_read )
     CASE (KDM6ADSCHEME)
          WRITE(0,*) 'KDM6AD_PHASE phy_init kdm6ad init case'
          FLUSH(0)
          WRITE(0,*) 'KDM6AD_PHASE phy_init before kdm6init'
          FLUSH(0)
          CALL kdm6init(rhoair0,rhowater,rhosnow,cliq,cpv,ccn_conc, &
                config_flags%hail_opt, allowed_to_read )
          WRITE(0,*) 'KDM6AD_PHASE phy_init after kdm6init'
          FLUSH(0)
     CASE (WDM7SCHEME)
...
'No bathymetry data detected and shalwater_depth not greater than 0.0. Re-run WPS to get bathymetry data or set shalwater_depth > 0.0')
  ...
  END SUBROUTINE shalwater_init
   ...
   SUBROUTINE vegparam_hc_init(MMINLU_loc,hc_jun,ivgtyp,its,itf,jts,jtf,ims,ime,jms,jme)

     CHARACTER*256, intent(in) :: MMINLU_loc
     real, dimension(ims:ime, jms:jme), intent(inout) :: hc_jun
     integer, intent(in) :: its,itf,jts,jtf,ims,ime,jms,jme
     integer, dimension(ims:ime, jms:jme), intent(in) :: ivgtyp

     real, dimension(1:50) :: SNUPTBL, RSTBL, RGLTBL, HSTBL, SHDTBL, MAXALB, &  
                             EMISSMINTBL, EMISSMAXTBL, LAIMINTBL, LAIMAXTBL, &
                             Z0MINTBL, Z0MAXTBL, ALBEDOMINTBL, ALBEDOMAXTBL, &
                             ZTOPVTBL,ZBOTVTBL
     INTEGER, dimension(1:50):: NROTBL
     INTEGER :: LUMATCH, IINDEX, LUCATS1, LCC
     integer :: i, j, lunit
     CHARACTER(LEN=256) :: LUTYPE1
     lunit = 37

     OPEN(lunit, FILE='VEGPARM.TBL',FORM='FORMATTED',STATUS='OLD')
 
     LUMATCH=0
     FIND_LUTYPE1 : DO WHILE (LUMATCH == 0)
       READ (lunit,*,END=993)
       READ (lunit,*,END=993)LUTYPE1
       READ (lunit,*)LUCATS1,IINDEX
       
 
       IF(LUTYPE1.EQ.MMINLU_loc)THEN
        print*, 'LANDUSE TYPE = ' // TRIM ( LUTYPE1 ) // ' FOUND', LUCATS1,' CATEGORIES'
         LUMATCH=1
        ELSE
          DO LCC = 1, LUCATS1+36
            read(lunit,*)
...
993  CONTINUE
     do j = jts, jtf
       do i = its, itf
         HC_jun(i,j) = ZTOPVTBL(IVGTYP(i,j))
       enddo 
      enddo

  END SUBROUTINE vegparam_hc_init

  SUBROUTINE tree_canopy_IGBP_init(treecoeff)
   INTEGER   :: lunit, ncat
   REAL      :: tcoeff(12)
   REAL    , INTENT(OUT)  :: treecoeff(12,21) 
   lunit = 37
   OPEN(lunit, FILE='TREECANOPY.TBL',FORM='FORMATTED',STATUS='OLD')
     read (lunit,*)
     read (lunit,*)
     read (lunit,*) ncat
     do j = 1,ncat
       READ (lunit,*) luinx, tcoeff(1:12)
       treecoeff(1:12,j) = tcoeff(1:12)
     end do
  close(lunit)

  END SUBROUTINE tree_canopy_IGBP_init
