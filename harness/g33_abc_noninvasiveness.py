...
import g33_abc_protocol as abcp
...
STATE_FIELDS = abcp.STATE_FIELDS
INCREMENT_FIELDS = abcp.INCREMENT_FIELDS
EXPECTED_FIELDS = abcp.EXPECTED_FIELDS
...
def parse_output(raw: bytes, algorithm: str, case_name: str) -> dict:
    """Validate the driver's complete raw-bit protocol (shared pure parser, so the
    live gate and the offline bundle verifier cannot drift)."""
    B, K = CASES[case_name]
    return abcp.parse_abc_output(raw, algorithm, case_name, B, K)
