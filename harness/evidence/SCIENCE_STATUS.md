...
| The `k = kte` slot is not consumed into any of the 197 output fields over one minute: correcting the loop alone, on production flags, changes 0 of 197 at every frame | CONFIRMED | `FINDING_ccn_init_reads_past_the_model_top_v1` |
...
| `HALO_EM_A` width-1 transfers match the owner bit for bit under the probe's MASS-grid ownership rule (`ips <= i <= iu` for every field): 12 exchanged fields x 3 interior i boundaries x both directions. `GROUPS` records each field's horizontal stagger but the parser does not yet use it, so the claim for the x- and y-staggered members (`ru`, `muu`, `rv`, `muv`) rests on that convention | CONFIRMED | `FINDING_i_seam_first_write_is_rk_step_prep_v1` |
...
| Three alternative builds -- both flags, `-fno-tree-vectorize` alone, `-ffp-contract=off` alone -- each remove the ENTIRE one-minute difference: 0 of 197 fields at 0/20/40/60 s, against 0/28/71/77 for production on the same case, frames and comparator. The bounds-checked build's difference carries the same STRUCTURAL SIGNATURE -- identical differing-field sets at 20 and 40 s, Jaccard 0.949 at 60 s, same `PH` band geometry -- while its cell masks, signs and magnitudes are uncompared | CONFIRMED | `FINDING_i_seam_is_code_generation_v1` |
