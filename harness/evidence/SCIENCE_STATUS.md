...
| Three alternative builds -- both flags, `-fno-tree-vectorize` alone, `-ffp-contract=off` alone -- each remove the ENTIRE one-minute difference: 0 of 197 fields at 0/20/40/60 s, against 0/28/71/77 for production on the same case, frames and comparator. The bounds-checked build's difference is the SAME seam, compared as field SETS: identical at 20 and 40 s, Jaccard 0.949 at 60 s with the four discrepancies all accumulation or capped-diagnostic fields | CONFIRMED | `FINDING_i_seam_is_code_generation_v1` |
