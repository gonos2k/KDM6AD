...
| `start_em.F:1786` initialises the CCN number with a loop to `kte` while reading `phb(i,k+1,j)`, so at `k = kte` it reads past the declared top; the physical mass levels 1..`kte-1` are in bounds and correct, and the value from the out-of-bounds read lands in the top ALLOCATED slot `k = kte`, whose consumers are unmeasured. The three sibling reads in the same file, and this block's own guard, all stop at `kte-1`. Corrected temporarily, it was the only bounds violation on the path and the seam persisted without it, so the two are independent | CONFIRMED | `FINDING_ccn_init_reads_past_the_model_top_v1` |
...
| No single `dyn_em` object, and no object's dependency closure, removes the seam: production 77, `{small_step_em, solve_em, madwrf}` 74, the nine-object `big_step_utilities` closure 75, those nine plus `advect_em` 74, and only their union of about 12 reaches 0. Each arm leaves the initial state unchanged (0 of 197 at frame 0) and is demonstrably active | CONFIRMED | `FINDING_i_seam_is_code_generation_v1` |
