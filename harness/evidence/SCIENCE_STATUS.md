...
| WHICH part of the positive-definite path fails for `qib` -- that it is reached and insufficient is measured; where the guarantee breaks is not. A paired moment advected independently of its mass has no reason to stay consistent with it, but that is a hypothesis | OPEN | `FINDING_invalid_qib_producer_v1` |
