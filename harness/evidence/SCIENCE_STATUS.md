...
| The first owned-cell difference inside the step is `ww` after `rk_step_prep`, two columns (i = 59, 176), BEFORE the `HALO_EM_A` exchange; timestep entry and RK-stage entry are clean | CONFIRMED | `FINDING_i_seam_first_write_is_rk_step_prep_v1` |
| `HALO_EM_A` delivers correctly for the width it declares: after the exchange the width-1 halo of its field set matches the owner bit for bit; columns beyond width 1 are stale by design | CONFIRMED | `FINDING_i_seam_first_write_is_rk_step_prep_v1` |
| The eastern-zone columns appear first in the `rk_tendency` outputs, one stage BEFORE `relax_bdy_dry`, so the lateral-boundary update does not make them | CONFIRMED | `FINDING_i_seam_first_write_is_rk_step_prep_v1` |
...
| Whether the halo `u`/`mu` columns `calc_ww_cp` reads actually differ at stage 1, on the i cut and on the j cut -- the measurement that would close or kill the unexchanged-halo account, and the one that would say whether 59 and 176 differ while 117 does not because their halo inputs do | OPEN | `FINDING_i_seam_first_write_is_rk_step_prep_v1` |
