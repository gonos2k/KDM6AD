...
| A decomposition that cuts only j (`1x2`, `1x3`, `1x4`) is bit-identical to `np=1` -- raw-word comparison, 0 of 197 fields differing in any bit -- so the difference is specific to splitting i. Its MECHANISM is now open: the inner-loop hypothesis below would explain it with no numerical defect anywhere | CONFIRMED | `FINDING_seam_is_i_specific_v1` |
...
| `ww` differs at the last owned mass column of the patches whose `i` loop runs 59 trips (0 and 2) and not at those running 58 (1 and 3), 4 of 4 | CONFIRMED AS A CLASS, not as a predictor: the no-contraction rebuild shows the difference is evaluation, and a trip count selects the vector body and remainder; WHICH boundaries differ has not been tested at another `nproc_x` | `FINDING_i_seam_first_write_is_rk_step_prep_v1` |
| Whether the ENTIRE i-cut phenomenon is decomposition-dependent code generation -- `i` is the INNER loop of these stencils, so cutting i changes inner trip counts while cutting j changes only the outer loop's extent | HYPOTHESIS; decisive test running (rebuild ALL of `dyn_em`, incl. `module_advect_em.F` which probe6 did NOT rebuild) | `FINDING_i_seam_first_write_is_rk_step_prep_v1` |
| The stage-5 `rk_tendency` difference survives the no-contraction build, but `module_advect_em.F` was outside the rebuilt object set, so it is NOT yet shown to be data either | UNMEASURED | `FINDING_i_seam_first_write_is_rk_step_prep_v1` |
