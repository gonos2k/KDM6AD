...
| No stencil propagates a stale halo value into an owned cell: across the probe7 pair, 15,316,767 halo cells differ from `np=1` at the same global column while 0 owned cells differ, at every stage. A stale read would be a DATA difference and would survive the flag change, so this carries to the production build | CONFIRMED (first RK stage of the first step, the dumped fields, owned cells at the anchors) | `FINDING_i_seam_is_code_generation_v1` |
