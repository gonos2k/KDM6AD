...
| Whether the instrumented binary produced the same trajectory as the deployed one -- restoring the source and recovering the deployed hash proves cleanup, not that the instrumented run matched | UNMEASURED | `FINDING_melt_arm_g5_and_number_policy_v1` |
...
| That `g4` floored 8 times and `g5` aborted 0 times in that run -- `rho0 > 900` is necessary not sufficient for the floor and the melt fraction was not recorded, `g5`'s guard tests the POST product `fl32(bg0*fl32(qg+/qg0))` not the pre-melt `bg0`, and the run was an unmodified-kernel counter run in which neither arm executed | REFUTED | `FINDING_melt_arm_g5_and_number_policy_v1` |
| The actual `g4` and `g5` candidates for the nine occurrences -- raw f32 `qg0`, `bg0`, `pgmlt`, the melt fraction, the effective density, and each arm's post-volume | UNMEASURED | `FINDING_melt_arm_g5_and_number_policy_v1` |
