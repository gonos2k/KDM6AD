...
| Nothing reads `scalar(:,kte,:,p_qnn)`: `flow_dep_bdy_qnn` loops to `kde-1` and `microphysics_driver` is called with `KTE = min(k_end, kde-1)`, and `p_qnn` appears nowhere else in `dyn_em`, `phys` or `share`. So the correction needs no padding policy, and the measured null effect has a source-level reason | CONFIRMED | `FINDING_ccn_init_reads_past_the_model_top_v1` |
