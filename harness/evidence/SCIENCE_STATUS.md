...
| One-time-initialisation reference (Arm C, the block removed) against the corrected code. It needs a kernel variant and a build, and its decision content is already settled: with `QNCCN = 0` on input -- which this case has -- A, B and C differ over tiling and halo, not over the field's origin, so what Arm C would add is the magnitude on corrected code, not the policy | UNMEASURED | `FINDING_ccn_onetime_reference_v1`, `FINDING_ccn_destroys_valid_input_v1` |
