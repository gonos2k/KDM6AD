...
| In the tested density-control matrices (legacy main chain and conservative ice chain), the moist-density-weighted residual changes sign and size with the imposed density profile. This does not establish a gradient-only formula for independently capped interfaces | CONFIRMED | `FINDING_density_falsification_v1`, `FINDING_ice_density_matrix_v1` |
...
| In the g4 density-recomputation branch (`rhox<=0`), for positive bg0 and a partial melt, the real-arithmetic floor condition is `a*rho0 >= rho_c`, where `rho_c=clip(qg0/max(bg0,tiny(f32)),100,900)`. Only with `bg0 >= tiny` does it require `rho0 > 900`; below tiny, qg0=5e-37, bg0=1e-39, a=0.3 floors at raw rho0 about 500. Rounded f32 boundary candidates must be checked directly | CONFIRMED | `FINDING_melt_arm_g5_and_number_policy_v1` |
