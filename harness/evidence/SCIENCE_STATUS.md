...
| `relax_bdy_dry` is not the first producer of the eastern-zone columns: they are already in the `rk_tendency` outputs one stage earlier | CONFIRMED | `FINDING_i_seam_first_write_is_rk_step_prep_v1` |
...
| Rebuilding the 30 LISTED `dyn_em` objects with `-O2 -fno-tree-vectorize -ffp-contract=off` removes every instrumented first-RK-stage difference between `np=1` and `4x1`: 0 of 141,214,320 f32 words | CONFIRMED | `FINDING_i_seam_is_code_generation_v1` |
| Which compiler transformation removes it -- vectorization, unrolling and contraction changed together | UNMEASURED | `FINDING_i_seam_is_code_generation_v1` |
| Whether a minimal-object rebuild that leaves the initial state bitwise unchanged also removes it; the 30-object build moved the state from stage 0 because `start_em` is in it | UNMEASURED | `FINDING_i_seam_is_code_generation_v1` |
