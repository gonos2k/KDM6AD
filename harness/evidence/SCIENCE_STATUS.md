...
| `g4` floors to zero exactly when `a*rho0 >= rho_c`, so `rho0 > 900` is NECESSARY but not SUFFICIENT -- at `rho0 > 900` it also needs `a >= 900/rho0`, and `rho0 = 1000, a = 0.1` does not floor. Predicate and `g4 == 0` agree on 100.000% of 200,000 draws | CONFIRMED | `FINDING_melt_arm_g5_and_number_policy_v1` |
...
| SCALAR ADVECTION produces the negative graupel volumes: at most 6 cells carry `bg < 0` entering a step, advection produces up to 80,586, and 2,570 remain when microphysics is called. Microphysics takes it back to 6. Counted at four boundaries in `solve_em.F`, `np=1`, ten minutes | CONFIRMED | `FINDING_invalid_qib_producer_v1` |
| The step chain is closed -- step N's RK entry equals step N-1's after-microphysics exactly in all three counters -- and the INITIAL CONDITION already holds 635 cells with `qg > 0`, `bg <= 0` and an admissible positive `bg` available, before any operator runs. Microphysics clears that to 6 in one step; the count then grows monotonically to 39 over thirty steps | CONFIRMED | `FINDING_invalid_qib_producer_v1` |
| Advection PRODUCES the negative volumes rather than amplifying inherited ones: `qib` is inside the positive-definite option (`scalar_adv_opt = 1`, `original = 0`), and a diagnostic arm that zeroes every negative `bg` at RK entry still sees up to 80,585 negative cells after advection against 80,586 unclamped, with every downstream count unchanged. The limiter is insufficient for this field, not bypassed | CONFIRMED | `FINDING_invalid_qib_producer_v1` |
...
| `qg` goes negative in the same stages and comparable numbers (19,849, 42,154, 1,172 surviving into microphysics under `moist_adv_opt = 1`), so this is NOT a paired-moment defect -- graupel MASS behaves the same way. The hypothesis that a moment advected independently of its mass drifts from it is withdrawn | REFUTED | `FINDING_invalid_qib_producer_v1` |
| That the surviving negatives come from the specified/relaxation zones -- bucketed by distance to the nearest lateral boundary over six steps on a 234x282x39 grid, the treated frame (`d <= 4`) is 7.67% of the cells and carries 8.32% of the negatives, an enrichment of 1.08x against the interior's 0.99x. Normalised, the two are the same, and 85% sit at `d >= 9` | REFUTED | `FINDING_invalid_qib_producer_v1` |
| WHY the final-stage renormalisation leaves about 1,500 negatives -- the update carries tendencies besides advection and couples through `mu_old`/`mu_new`, and which of those defeats the bound is not measured | OPEN | `FINDING_invalid_qib_producer_v1` |
| Of those 551 zero-volume cell-frames, 280 have `qg < 100*eta` (eta = 1.4012985e-45, the smallest positive f32 subnormal) so NO positive `bg` could have been admissible -- a representability failure of the paired moment; 90 admit only `bg = eta`; and 181 have `qg >= 900*eta` where many admissible `bg` exist, the largest at `qg = 5.1177e-18`. The 16 negative-volume cells are outside the argument | CONFIRMED | `FINDING_melt_arm_g5_and_number_policy_v1` |
...
| Sedimentation CREATES number every transfer, in the real model: each `k+1 -> k` move carries `(rho(k)-rho(k+1))*Dz(k+1)*falkn(k+1)*dt` into existence on the physical `rho*Dz` measure, because the flux has no density (`:1194`, `:1286`) while the incoming term carries `delz(k+1)/delz(k)` (`:1222`, `:1306`). Over 163,801,204 transfers in ten minutes at `np=1`: 4.6188% of the transferred RAIN number and 8.5710% of the transferred ICE number | CONFIRMED | `FINDING_number_creation_in_the_real_model_v1` |
| That ratio is created-per-transferred, NOT a growth rate of the domain number; and the `min(..., n(k+1))` clip binds on 2.91% of ice transfers, so the ice figure is an upper bound to that extent while the rain figure (clip 0.0018%) is effectively exact | CONFIRMED | `FINDING_number_creation_in_the_real_model_v1` |
