...
| `G33-NCMIN-004` | **active** | confirmed-with-scope | g33_fixture_boundary_mapping_v1, one call, nsplit=1. Operator and physical mass weighting are BOTH computed; dtcld is applied; the decomposition closes against micro_pre/post_state_update. This scope previously said the opposite of all three -- it described the first version of the ledger, which the text above had already corrected (owner §7.1). . What the figure IS: a source-order direct arithmetic share on a one-call synthetic fixture. What it is NOT, each ruled out for its own reason: not a path-independent process fraction, because F is non-linear and the split moves with the substitution order; not a causal mediation fraction, because pracw consumes the rain praut produces (G33-NCMIN-005); not an adjoint sensitivity, because nothing here differentiates the operator. |
| `G33-NCMIN-005` | **hold** | open-question | needs a mediation experiment, not another reading of this fixture: one arm per link of the chain, each holding that link at the local-oracle value with the rest of the run unchanged. The adjoint form of the same question -- d(q_r+)/d(ncmin) through each path -- needs the forcing VJP, which is not implemented. |
