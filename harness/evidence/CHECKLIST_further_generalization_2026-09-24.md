...
| F6 | **Bounded accepted-ledger pilot complete** | Accepted interval, rollback and restart | X5's independent two-interval plan produces integrated `0.4+6.4=6.8 J/m²`; an attempted `10 J/m²` trial is rejected without accumulation before a retry is accepted. Five new test functions (18 collected cases with X5) check exact accepted prefix, duplicate event idempotence, same-ID content conflict, immutable trial rejection, dependency/unit binding and checkpoint restore without reapplication. This is pre-acceptance rollback only; it does not close G4's measured host restart divergence. |
| F7 | **Bounded representation pilot complete** | Resolution and stochastic meaning | Six new synthetic test functions (28 collected cases with retained X6/X10 tests) show radii `[1,3] µm` have mean radius `2`, its cube `8`, but weighted third moment `14 µm³`; a cubic-moment claim requires an explicit retained value or named closure result. Internal stochastic amounts must pair under an independent realization-ID plan, so zero ensemble-mean imbalance cannot hide per-member loss. Analysis/ML corrections use X10's separate external-inventory calculation. No actual closure, stochastic KDM or DA/ML execution is certified. |
...
F6 evidence: `REPORT_further_generalization_accepted_ledger_2026-09-25.md`,
`accepted_interval_ledger.py` and its focused tests. Its checkpoint is an
in-memory verification record, not a KDM host restart file.
