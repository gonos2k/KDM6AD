...
| S4 | **OPEN** | Native dynamic `mstep ≥ 2` consumption | Capture an actual completed column with at least two ice substeps and changed post-reslope velocity where possible. Prove each consumer uses the latest `v/dz` once, selection versus consumption is recorded, finished columns remain inactive, and restart does not reuse stale coefficients. Synthetic G1 and all-one-mstep G4 census do not close this. |
