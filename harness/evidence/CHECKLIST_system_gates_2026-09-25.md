...
| S16 | **OPEN — D4** | One-ULP D2–D4 freeze-heat attribution | At the first actual Fortran/C++ stored-`T` difference, capture the pre-heat state, applied freeze amounts/rates, `xlf`, `cpm`, branch masks and each source-ordered f32/f64 intermediate. Reproduce the one-ULP result independently and distinguish a differing operand from operation order or rounding before approving parity. Do not fold this into C4 G3.3's different legacy ULP-envelope comparison or relax either gate. |
