...
def verify_offline_replay(run):
    """Run the complete backend-neutral ladder over this raw Fortran stream.

    The former local check grouped updates by ``(col, k, n)`` and therefore
    overwrote records from later outer loops.  Normalization preserves the full
    ``(loop, chain, n, col, k)`` identity and :mod:`g33_replay` checks every
    relation and every loop.  Keep this public name for existing callers, but
    make all of them use the same complete gate.
    """
    import g33_normalize as _normalize
    import g33_replay as _replay
    try:
        return _replay.replay_run(_normalize.from_fortran_run(run))
    except Exception as exc:
        if isinstance(exc, FortranRunError):
            raise
        raise FortranRunError(f"full offline replay failed: {exc}") from None
