...
def test_no_generated_PERMTEST_fixture_is_TRACKED():
    """`_run` writes its rendered fixture into the repo -- the build selects a
    module by NAME from its own directory, so it has to sit beside the others
    -- and removes it in a `finally`. A KILLED run never reaches that finally.

    The orphan then makes the working tree dirty, and every producer that
    refuses to build a bundle from a dirty tree errors: it cost a full gate as
    309 unrelated errors that reproduced in no single file. Then a `git add -A`
    committed the orphan, which made it permanent -- now the file's own cleanup
    showed up as a TRACKED DELETION, so the tree was dirty whether the test had
    run or not (Codex).

    Nothing generated by a test belongs in the index.
    """
    tracked = subprocess.run(
        ["git", "ls-files", "harness/g33_fortran/g33_fixture_permtest_*",
         "harness/g33_overlay/g33_fixture_permtest_*"],
        cwd=ROOT, capture_output=True, text=True).stdout.split()
    assert not tracked, (
        f"generated permtest fixtures are in the index: {tracked}. They are "
        f"rendered per run and unlinked in a `finally`, so tracking one makes "
        f"its own cleanup a deletion the dirty-tree guards then refuse")
