...
def test_a_pre_decision_version_stream_is_never_verdict_ready(tmp_path):
    ...
    root = _bundle(tmp_path / "b")
    leg = fbio.verify_fortran_bundle(root, "legacy", **_anchors(root))
    # The current decision version, not a literal: pinning it here meant the
    # test failed on every bump for the bump's own sake, which trains the
    # reader to edit the number rather than read what it is asserting.
    assert leg.verdict_ready
    assert leg.run.protocol_version == fbio.DECISION_PROTOCOL_VERSION
    for older in range(1, fbio.DECISION_PROTOCOL_VERSION):
