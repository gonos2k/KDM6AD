#!/bin/bash
# Enforced G3.3-M gate: substep, surface, shared fixture, and C++ A/B/C.
set -u
cd "$(dirname "$0")/../.."
...
mut_out=$(python3 harness/g33_selfcheck.py --driver "$OUT/mutant/selfcheck_driver" 2>&1); mut_rc=$?
cons_out=$(python3 harness/g33_selfcheck.py --driver "$OUT/mutant_cons/selfcheck_driver" 2>&1); cons_rc=$?
prevout_out=$(python3 harness/g33_selfcheck.py --driver "$OUT/mutant_prevout/selfcheck_driver" 2>&1); prevout_rc=$?
poststate_out=$(python3 harness/g33_selfcheck.py --driver "$OUT/mutant_poststate/selfcheck_driver" 2>&1); poststate_rc=$?
fallacc_out=$(python3 harness/g33_selfcheck.py --driver "$OUT/mutant_fallacc/selfcheck_driver" 2>&1); fallacc_rc=$?
...
for pin in "$LEGACY_LINE" "$CONS_LINE"; do
    if ! printf '%s\n' "$real_out" | grep -qF "$pin"; then
        echo "KILL GATE FAIL: substep coverage drifted; missing: $pin"
...
if ! why=$(verdict_mutant "$mut_out" "$mut_rc" "$EXPECTED_KILL"); then
    echo "KILL GATE FAIL: shadow mutant not the controlled predicted kill: $why"; exit 1
fi
...
if ! why=$(verdict_mutant "$cons_out" "$cons_rc" "$CONS_KILL"); then
    echo "KILL GATE FAIL: conservative inflow mutant not the predicted kill: $why"; exit 1
fi
...
if ! why=$(verdict_mutant "$prevout_out" "$prevout_rc" "$PREVOUT_KILL"); then
    echo "KILL GATE FAIL: prev_out mutant not the predicted kill: $why"; exit 1
fi
...
if ! why=$(verdict_mutant "$poststate_out" "$poststate_rc" "$POSTSTATE_KILL"); then
    echo "KILL GATE FAIL: poststate mutant not the predicted kill: $why"; exit 1
fi
...
if ! why=$(verdict_mutant "$fallacc_out" "$fallacc_rc" "$FALLACC_KILL"); then
    echo "KILL GATE FAIL: fallacc mutant not the predicted kill: $why"; exit 1
fi
...
surface_omit_out=$(python3 harness/g33_surface_selfcheck.py --driver "$OUT/mutant_surface_omit_qi/surface_selfcheck_driver" --algorithm conservative 2>&1); surface_omit_rc=$?
surface_bottom_out=$(python3 harness/g33_surface_selfcheck.py --driver "$OUT/mutant_surface_wrong_bottom/surface_selfcheck_driver" --algorithm conservative 2>&1); surface_bottom_rc=$?
echo "$surface_out" | tail -3
if [ "$surface_rc" -ne 0 ]; then
    echo "KILL GATE FAIL: real surface driver did not PASS (rc=$surface_rc)"; exit 1
fi
for pin in \
  ...
  "SURFACE SELF-CHECK PASS"; do
    printf '%s\n' "$surface_out" | grep -qF "$pin" || {
        echo "KILL GATE FAIL: surface coverage drifted; missing: $pin"; exit 1; }
done
SURFACE_OMIT_KILL="FAIL surface-offline: conservative L1_surface rain_increment"
verdict_mutant "$surface_omit_out" "$surface_omit_rc" "$SURFACE_OMIT_KILL" >/dev/null || {
    echo "KILL GATE FAIL: surface omit-qi mutant missed"; exit 1; }
SURFACE_BOTTOM_KILL="FAIL surface-link: conservative L1_surface bottom_fall_qr != L1_main_n2 QR_FALLACC(k=3).fall_after"
verdict_mutant "$surface_bottom_out" "$surface_bottom_rc" "$SURFACE_BOTTOM_KILL" >/dev/null || {
    echo "KILL GATE FAIL: surface wrong-bottom mutant missed"; exit 1; }
...
# ── Existing full-step C++ A/B/C non-invasiveness ───────────────────────────
...
echo "G3.3 CONTINUOUS GATE PASS: substep+surface, seven mutants, shared fixture, C++ A/B/C"
